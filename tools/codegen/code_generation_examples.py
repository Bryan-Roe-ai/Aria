...
def example_5_text_statistics():
    ...
    for text in sample_texts:
        ...
        print(f"  Text: '{text}'")
        print(f"  Words: {stats['word_count']}, Chars: {stats['char_count']}, Avg Length: {stats['avg_word_length']:.2f}\n")
...
def example_7_batch_generation():
    ...
    maker = ToolMaker()
    functions = [
        ('celsius_to_fahrenheit', 'Convert Celsius to Fahrenheit', {'celsius': 'float'}, {'fahrenheit': 'float'}),
        ('is_prime', 'Check if a number is prime', {'n': 'int'}, {'is_prime': 'bool'}),
        ('reverse_list', 'Reverse a list of items', {'items': 'list'}, {'reversed': 'list'}),
    ]
