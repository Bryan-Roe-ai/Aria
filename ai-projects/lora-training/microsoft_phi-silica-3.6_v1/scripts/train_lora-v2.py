...
try:
    ...
    raise SystemExit(
        "pyyaml is required. Install with: pip install pyyaml") from None
try:
    ...
    AutoModelForCausalLM = AutoTokenizer = Trainer = TrainingArguments = None  # type: ignore
...
def build_text_from_messages(messages: list[dict[str, str]]) -> str:
    ...
    Uses improved formatting and end tokens.
    """
    parts: list[str] = []
    for m in messages:
...
def make_hf_dataset_from_files(
    ...
    if load_dataset is None:
        raise RuntimeError(
            ...
            "Install 'datasets' to load datasets."
        )
...
def _validated_remote_url(path_or_url: str) -> str:
    ...
    if parsed.scheme not in ("http", "https"):
        raise ValueError(
            f"Unsupported URL scheme for manifest source: {parsed.scheme!r}")
    if not parsed.hostname:
        raise ValueError("Remote manifest URL must include a hostname")

    allowed_hosts = {h.strip().lower() for h in os.environ.get(
        "LORA_MANIFEST_ALLOWED_HOSTS", "").split(",") if h.strip()}
    host = parsed.hostname.lower()
    if allowed_hosts and host not in allowed_hosts:
        raise ValueError(
            f"Remote manifest host {host!r} "
            "is not in LORA_MANIFEST_ALLOWED_HOSTS"
        )

    try:
        addrinfos = socket.getaddrinfo(parsed.hostname, parsed.port or (
            443 if parsed.scheme == "https" else 80), type=socket.SOCK_STREAM)
    except socket.gaierror as e:
        raise ValueError(
            f"Could not resolve remote manifest host "
            f"{parsed.hostname!r}: {e}"
        ) from e
    ...
    for ai in addrinfos:
        ...
        if (
            ...
            raise ValueError(
                "Remote manifest host resolves to disallowed address: "
                f"{ip_str}"
            )
...
def main():
    ...
    if train_manifest or eval_manifest:
        ...
        dataset_path = (
            Path(args.dataset) if args.dataset else resolve_path(
    ...
    if streaming_train:
        ...
        steps_per_epoch = max(
            1, math.ceil(target_samples /
                         max(1, cfg.finetune_train_batch_size))
        )
