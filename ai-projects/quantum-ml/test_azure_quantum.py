...
def test_optimized_circuit_on_hardware(azure: AzureQuantumIntegration, backend_name: str = None):
    ...
    try:
        ...
        # Submit to hardware
        print(
            f"Submitting optimized circuit to {backend_name or 'default backend'}...")
        job = azure.submit_circuit(
            circuit,
            backend_name=backend_name,
            shots=500,
        ...
        entropy = -sum((count/total) * np.log2(count/total)
                       for count in results['counts'].values() if count > 0)
...
def main():
    ...
    if bell_results:
        ...
        proceed = input(
            "Proceed with optimized circuit test? (yes/no): ").strip().lower()
        if proceed == "yes":
