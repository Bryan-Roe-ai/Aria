...
def main() -> None:
    ...
    findings_hint = f" ({', '.join(findings)})" if findings else ""
    if _BLOCK:
        print(
            f"🛑  SECRETS BLOCKED: Detected possible hardcoded secret{findings_hint}{path_hint}. "
        ...
        print(
            f"⚠️  SECRETS WARNING: Possible hardcoded secret detected{findings_hint}{path_hint}. "
