...
**NEW**: Automated repo improvement workflow — continuous code quality fixes and health checks with `python run_automation.py --auto-improve --strict-endpoints` or `python run_continuous_automation.py --interval 60 --strict-endpoints`
...
```bash
# === AUTO-IMPROVE REPO ===
# One-shot repo auto-improve (ruff fixes + health checks + endpoint validation)
python run_automation.py --auto-improve --strict-endpoints

# Direct health orchestrator (integrated custom validation + alert dispatch)
python scripts/repo_health_automation.py --once --strict-endpoints

# Local pre-commit auto-improve hook entrypoint
python scripts/auto_improve_pre_commit_hook.py --fix

# Standalone custom validation rules and health alerts
python scripts/custom_validation_rules.py --run
python scripts/health_check_alerts.py --check-now

# Continuous auto-improve daemon (every 60 minutes by default; logs to logs/continuous_automation.log)
...
| Repo health automation (integrated) | `scripts/repo_health_automation.py` |
| Auto-improve pre-commit hook | `scripts/auto_improve_pre_commit_hook.py` |
| Custom validation rules framework | `scripts/custom_validation_rules.py` |
| Health check alerting | `scripts/health_check_alerts.py` |
