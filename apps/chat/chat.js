...
console.log(
    "chat.js loaded - v2025-11-21-qai - Provider: QAI auto-detect with quantum mode",
)
;(function window_qai_chat_client() {
    ...
    function isSyntheticCompactionPlaceholder(content) {
        ...
        return (
            ...
            normalizedLines.every(line =>
                COMPACTION_PLACEHOLDER_LINES.has(line),
            )
        )
    }
    ...
    function initEmbeddedTransport() {
        console.log(
            "chat.js: ARIA_EMBEDDED — wiring chat transport (send + AGI)",
        )
        ...
        if (agiModeButton)
            agiModeButton.addEventListener("click", toggleAgiMode)
        if (cancelStreamBtn) {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const ariaAvatarContainer = document.getElementById(
            "ariaAvatarContainer",
        )
        const ariaAvatarClose = document.getElementById("ariaAvatarClose")
        const ariaAvatarRegenerate = document.getElementById(
        ...
        if (ariaAvatarRegenerate) {
            ariaAvatarRegenerate.addEventListener("click", () =>
                generateAriaAvatar(true),
            )
        }
    ...
    async function sendMessage() {
        ...
        emitEmbeddedChatEvent("aria-chat-message", {
            role: "user",
            content: text,
        })
        messageInput.value = ""
        ...
        try {
            ...
            if (retryCount < MAX_RETRIES && error.message.includes("HTTP")) {
                ...
                addMessage(
                    "system",
                    `⚠️ Request failed. Retrying in ${delay / 1000}s... (${retryCount}/${MAX_RETRIES})`,
                )
    ...
    async function oneShotResponse(typingIndicator) {
        ...
        const apiMessages = sanitizeConversationMessages(
            systemPrompt
    ...
    async function previewAgiRouting() {
        ...
        const query =
            lastUser && lastUser.content ? String(lastUser.content).trim() : ""
        if (!query) {
        ...
        try {
            ...
            const data = await response.json()
            const agent =
                (data.routing && data.routing.selected_agent) || "general"
    ...
    function renderAgiDeltaHtml(delta) {
        if (
            typeof globalThis.AGIStreamUtils !== "undefined" &&
            globalThis.AGIStreamUtils.prettyPrintDelta
        ) {
    ...
    function newChat() {
        ...
        if (
            messages.length > 0 &&
            !confirm("Start a new chat? Current conversation will be cleared.")
        ) {
    ...
    function exportChat() {
        if (messages.length === 0) {
            if (typeof showToast === "function")
                showToast("No messages to export", 3000)
            return
        }
        ...
        input.onchange = e => {
            ...
            reader.onload = event => {
                try {
                    ...
                    if (typeof showToast === "function")
                        showToast(`Import failed: ${error.message}`, 4000)
                }
            }
    ...
    async function generateAriaAvatar(regenerate = false) {
        ...
        try {
            ...
            const prompt =
                "Portrait of Aria, anime-style AI assistant character, purple gradient hair, cute anime girl, friendly expression, digital art, high quality, detailed, vibrant colors, soft lighting"
            ...
            if (result.image_url || result.image_data) {
                ...
                if (regenerate) {
                    addMessage(
                        "assistant",
                        "✨ How do I look? I just got a fresh new appearance from the AI! 💜",
                    )
                }
            } else {
    ...
    async function handleImageUpload(event) {
        ...
        `
            ...
            document
                .getElementById("visionClearButton")
                .addEventListener("click", clearVisionUpload)
    ...
    function saveToStorage() {
        try {
            localStorage.setItem(
                "chatMessages",
                JSON.stringify(sanitizeConversationMessages(messages)),
            )
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.available) {
                    ...
                    updateStatus(
                        "Quantum backend unavailable - using classical fallback",
                    )
                }
            }
        } catch (error) {
    ...
    async function performQuantumAnalysis(text) {
        ...
        try {
            ...
            await fetchCircuitVisualization(
                data.quantum_state.n_qubits,
                data.quantum_state.n_layers,
            )
