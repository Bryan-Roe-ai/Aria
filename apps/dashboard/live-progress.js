...
class LiveProgressTracker {
    ...
    async pollProgress() {
        ...
        try {
            const response = await fetch(
                `/api/job-progress/${this.activeJobId}`,
            )
            if (!response.ok) {
    ...
    handleStatusUpdate(statusData) {
        ...
        const job = statusData.jobs.find(
            j => j.id === this.activeJobId || j.name === this.activeJobId,
        )
        if (!job) return
    ...
    updateProgressBars(data) {
        ...
        if (epochBar) {
            const epochPercent =
                ((data.current_step || 0) /
                    (data.total_steps_per_epoch || 100)) *
                100
    ...
    updateMetrics(data) {
        ...
        if (lossEl && data.current_loss !== undefined) {
            ...
            lossEl.className =
                "metric-value " + this.getLossClass(data.current_loss)
        }
    ...
    async pauseTraining() {
        ...
        try {
            const response = await fetch(
                `/api/job-control/${this.activeJobId}/pause`,
                {
                    method: "POST",
                },
            )
            ...
            if (response.ok) {
                if (typeof showToast === "function")
                    showToast("Training paused ✓", "ok", 3000)
            }
        } catch (err) {
            console.error("[LiveProgress] Pause error:", err)
            if (typeof showToast === "function")
                showToast("Failed to pause training", "warn", 4000)
        }
    }
    ...
    async stopTraining() {
        ...
        try {
            const response = await fetch(
                `/api/job-control/${this.activeJobId}/stop`,
                {
                    method: "POST",
                },
            )
            ...
            if (response.ok) {
                this.stopTracking()
                if (typeof showToast === "function")
                    showToast("Training stopped", "ok", 3000)
                this.hideProgressUI()
            }
        } catch (err) {
            console.error("[LiveProgress] Stop error:", err)
            if (typeof showToast === "function")
                showToast("Failed to stop training", "warn", 4000)
        }
    }
