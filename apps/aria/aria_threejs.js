...
;(function AriaThreeJSEngine() {
    ...
    function initEngine(stageEl) {
        ...
        const ro = new ResizeObserver(() => {
            const w = stageEl.offsetWidth,
                h = stageEl.offsetHeight
            renderer.setSize(w, h, false)
            camera.aspect = w / h
            camera.updateProjectionMatrix()
        })
        ro.observe(stageEl)
        const w0 = stageEl.offsetWidth,
            h0 = stageEl.offsetHeight
        ...
        window.aria3D = {
            ...
            setStageTheme(stageStyle) {
                ...
                const isQuantum =
                    label.includes("quantum") || accent === "#38bdf8"
                ...
                if (envRefs.floor && envRefs.floor.material) {
                    envRefs.floor.material.color.set(
                        isQuantum ? 0x1e293b : 0x7cbf8a,
                    )
                }
            ...
            triggerDance: () => {
                anim.dancing = !anim.dancing
            },
            triggerJump: () => {
                if (anim.jumpT < 0) anim.jumpT = 0
            },
            triggerWave: () => {
                anim.waveT = 0
            },
    ...
    function buildEnvironment(scene, M, renderer) {
        ...
        const envTexture = new THREE.TextureLoader().load(
            "https://cdn.jsdelivr.net/gh/mrdoob/three.js@r152/examples/textures/2294472375_24a3b8ef46_o.jpg",
            texture => {
        ...
        const tableTop = new THREE.Mesh(
            new THREE.BoxGeometry(1.95, 0.12, 0.92),
            M.table,
        )
        ...
        const legGeo = new THREE.BoxGeometry(0.1, 0.82, 0.1)
        ;[
            [-0.82, -0.47, -0.32],
            [0.82, -0.47, -0.32],
            [-0.82, -0.47, 0.32],
    ...
    function buildCharacter(scene, M) {
        ...
        const waist = new THREE.Mesh(
            new THREE.CylinderGeometry(0.155, 0.165, 0.18, 20),
            M.shirt,
        )
        waist.position.y = 0
        ...
        const torsoMesh = new THREE.Mesh(
            new THREE.CylinderGeometry(0.182, 0.148, 0.46, 22),
            M.shirt,
        )
        ...
        skirtProfile.forEach(([r, y]) =>
            skirtPoints.push(new THREE.Vector2(r, y)),
        )
        const skirtMesh = new THREE.Mesh(
            new THREE.LatheGeometry(skirtPoints, 28),
            M.skirt,
        )
        ...
        const bowL = new THREE.Mesh(
            new THREE.BoxGeometry(0.055, 0.032, 0.02),
            M.bow,
        )
        bowL.position.set(-0.03, 0.56, 0.1)
        bowL.rotation.z = 0.35
        const bowR = new THREE.Mesh(
            new THREE.BoxGeometry(0.055, 0.032, 0.02),
            M.bow,
        )
        bowR.position.set(0.03, 0.56, 0.1)
        ...
        ;[bowL, bowR, bowCenter].forEach(b => {
            b.castShadow = true
            torsoGroup.add(b)
        })

        /* ── Arms ── */
        const shoulderGroupL = new THREE.Group()
        shoulderGroupL.name = "shoulderL"
        shoulderGroupL.position.set(-0.22, 0.5, 0)
        const armL = buildArm("L", M)
        shoulderGroupL.add(armL)
        torsoGroup.add(shoulderGroupL)

        const shoulderGroupR = new THREE.Group()
        shoulderGroupR.name = "shoulderR"
        shoulderGroupR.position.set(0.22, 0.5, 0)
        ...
        const neck = new THREE.Mesh(
            new THREE.CylinderGeometry(0.06, 0.072, 0.16, 16),
            M.skin,
        )
    ...
    function buildHead(M) {
        ...
        const skull = new THREE.Mesh(
            new THREE.SphereGeometry(0.225, 32, 24),
            M.skin,
        )
        ...
        const backHair = new THREE.Mesh(
            new THREE.SphereGeometry(0.205, 20, 18),
            M.hair,
        )
        ...
        const bangData = [
            ...
            { x: 0.0, y: 0.125, rz: 0.0, sx: 0.56, sy: 1.55, sz: 0.86 },
            { x: 0.09, y: 0.12, rz: -0.22, sx: 0.48, sy: 1.35, sz: 0.82 },
        ]
        bangData.forEach(d => {
        ...
        const upperLip = new THREE.Mesh(
            new THREE.TorusGeometry(0.04, 0.01, 6, 14, Math.PI),
            M.lips,
        )
        upperLip.position.set(0, 0, 0)
        upperLip.rotation.z = Math.PI
        const lowerLip = new THREE.Mesh(
            new THREE.TorusGeometry(0.038, 0.012, 6, 14, Math.PI),
            M.lips,
        )
    ...
    function buildEye(side, M) {
        ...
        const sclera = new THREE.Mesh(
            new THREE.SphereGeometry(0.051, 18, 14),
            M.eyeWhite,
        )
        ...
        const irisRing = new THREE.Mesh(
            new THREE.TorusGeometry(0.02, 0.009, 6, 16),
            M.eyeIris2,
        )
        ...
        shine2.position.set(0.01, -0.008, 0.042)
    ...
    function buildArm(side, M) {
        ...
        uaGrp.name = `upperArm_${side}`
        uaGrp.position.y = -0.08
        const ua = new THREE.Mesh(
            new THREE.CapsuleGeometry(0.052, 0.2, 6, 14),
            M.skin,
        )
        ua.position.y = -0.1
        ua.castShadow = true
        ...
        const thumb = new THREE.Mesh(
            new THREE.CapsuleGeometry(0.016, 0.032, 4, 8),
            M.skin,
        )
    ...
    function buildLeg(side, M) {
        ...
        /* Upper leg (thigh) */
        const ulGrp = new THREE.Group()
        ulGrp.name = `upperLeg_${side}`
        const ul = new THREE.Mesh(
            new THREE.CapsuleGeometry(0.066, 0.26, 8, 16),
            M.pants,
        )
        ul.position.y = -0.13
        ul.castShadow = true
        ...
        /* Lower leg (calf/shin) */
        const llGrp = new THREE.Group()
        llGrp.name = `lowerLeg_${side}`
        llGrp.position.y = -0.275
        const ll = new THREE.Mesh(
            new THREE.CapsuleGeometry(0.056, 0.22, 8, 14),
            M.skin,
        )
        ll.position.y = -0.11
        ll.castShadow = true
        ...
        const foot = new THREE.Mesh(
            new THREE.BoxGeometry(0.1, 0.065, 0.22),
            M.shoes,
        )
        ...
        const sole = new THREE.Mesh(
            new THREE.BoxGeometry(0.098, 0.018, 0.218),
            M.soles,
        )
    ...
    function syncFromCSS(char, anim, stageEl) {
        ...
        if (cl.contains("mood-happy")) {
            anim.moodColor.set(0x5fb3f5)
            anim.moodIntensity = 0.4
        } else if (cl.contains("mood-excited")) {
            anim.moodColor.set(0xffcc00)
            anim.moodIntensity = 0.6
        } else if (cl.contains("mood-sad")) {
            anim.moodColor.set(0x6c7a89)
            anim.moodIntensity = 0.25
        } else if (cl.contains("mood-angry")) {
            anim.moodColor.set(0xe74c3c)
            anim.moodIntensity = 0.5
        } else if (cl.contains("mood-calm")) {
            anim.moodColor.set(0x4caf50)
            anim.moodIntensity = 0.3
        } else if (cl.contains("mood-thinking")) {
            anim.moodColor.set(0x9b59b6)
            anim.moodIntensity = 0.3
        } else {
            anim.moodIntensity = Math.max(0, anim.moodIntensity - 0.01)
        }
    }
    ...
    function updateCharacter(char, anim, dt, mouse2D, camera) {
        ...
        char.moodLight.intensity +=
            (anim.moodIntensity - char.moodLight.intensity) * 0.08
    ...
    function updateBlink(char, anim, dt) {
        ...
        if (
            anim.blinkState === 0 &&
            anim.blinkTimer > IDLE_BLINK_INTERVAL * (0.8 + Math.random() * 0.4)
        ) {
            anim.blinkState = 1
            anim.blinkTimer = 0
        }
    ...
    function updateIdleLimbs(char, anim) {
        ...
        /* gentle arm float */
        char.shoulderGroupL.children[0].uaGrp.rotation.x =
            Math.sin(t * 0.8) * 0.04
        char.shoulderGroupR.children[0].uaGrp.rotation.x =
            Math.sin(t * 0.8 + 0.5) * 0.04
        char.shoulderGroupL.children[0].uaGrp.rotation.z =
            -0.18 + Math.sin(t * 0.6) * 0.03
        char.shoulderGroupR.children[0].uaGrp.rotation.z =
            0.18 - Math.sin(t * 0.6 + 0.3) * 0.03
        /* gentle body sway */
    ...
    function updateWalk(char, anim, dt) {
        ...
        /* Torso bob */
        char.torsoGroup.position.y =
            0.82 + Math.abs(Math.sin(t * 2)) * (anim.isRunning ? 0.05 : 0.03)
    ...
    function updateDance(char, anim, dt) {
        ...
        char.headGroup.rotation.z = s * 0.1
        ...
        char.legL.ulGrp.rotation.x = s2 * 0.3
        char.legR.ulGrp.rotation.x = -s2 * 0.3
    ...
    function updateWave(char, anim, dt) {
        ...
        if (p <= 1.0) {
            ...
            if (char.shoulderGroupR.children[0].laGrp)
                char.shoulderGroupR.children[0].laGrp.rotation.x =
                    -0.3 + wave * 0.6
        } else {
            resetArmIdle(char)
            anim.waveT = -1
    ...
    function updateExpression(char, anim, dt) {
        ...
        switch (target) {
            ...
            case "happy": {
                ...
                head.eyebrowL.rotation.z +=
                    (0.28 - head.eyebrowL.rotation.z) * speed
                head.eyebrowR.rotation.z +=
                    (-0.28 - head.eyebrowR.rotation.z) * speed
                head.eyebrowL.position.y +=
                    (0.125 - head.eyebrowL.position.y) * speed
                head.eyebrowR.position.y +=
                    (0.125 - head.eyebrowR.position.y) * speed
                break
            }
            case "sad": {
                ...
                head.eyebrowR.rotation.z +=
                    (0.28 - head.eyebrowR.rotation.z) * speed
                head.eyebrowL.position.y +=
                    (0.105 - head.eyebrowL.position.y) * speed
                head.eyebrowR.position.y +=
                    (0.105 - head.eyebrowR.position.y) * speed
                break
            }
            case "surprised": {
                mouth.scale.set(0.9, 0.7, 1)
                if (head.eyeL && head.eyeL.lid)
                    head.eyeL.lid.scale.y +=
                        (1.3 - head.eyeL.lid.scale.y) * speed
                if (head.eyeR && head.eyeR.lid)
                    head.eyeR.lid.scale.y +=
                        (1.3 - head.eyeR.lid.scale.y) * speed
                head.eyebrowL.position.y +=
                    (0.14 - head.eyebrowL.position.y) * speed
                head.eyebrowR.position.y +=
                    (0.14 - head.eyebrowR.position.y) * speed
                break
            }
            default: {
                /* neutral – restore */
                ...
                mouth.position.y += (-0.077 - mouth.position.y) * speed
                head.eyebrowL.rotation.z +=
                    (0.12 - head.eyebrowL.rotation.z) * speed
                head.eyebrowR.rotation.z +=
                    (-0.12 - head.eyebrowR.rotation.z) * speed
                head.eyebrowL.position.y +=
                    (0.115 - head.eyebrowL.position.y) * speed
                head.eyebrowR.position.y +=
                    (0.115 - head.eyebrowR.position.y) * speed
            }
        }
    }
    ...
    function updateParticles(scene, time, dt) {
        ...
        scene.children.forEach(c => {
            ...
            for (let i = 0; i < pos.count; i++) {
                pos.setY(
                    i,
                    base[i * 3 + 1] + Math.sin(time * 0.6 + i * 1.3) * 0.06,
                )
