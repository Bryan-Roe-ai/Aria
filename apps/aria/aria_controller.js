...
function applyServerObjectsState(objectsState) {
    ...
    for (const [objectId, objState] of Object.entries(objectsState)) {
        ...
        const hidden =
            state === "hidden" || state === "off_stage" || state === "removed"
...
async function loadQuantumStage(preset = "intro") {
    try {
        ...
        if (typeof window.setQuantumLiveStatus === "function") {
            window.setQuantumLiveStatus(
                "Quantum setup failed — is the Aria server running?",
                false,
            )
        }
        return null
...
document.addEventListener("mousemove", e => {
    ...
    _eyeRaf = requestAnimationFrame(() => {
        ...
        const oy =
            dist > 15 ? (dy / dist) * Math.min(maxO * 0.55, dist * 0.016) : 0
...
function showFeedback(message) {
    ...
    feedback.style.cssText =
        "position:absolute; top:20px; left:50%; transform:translateX(-50%); background:var(--accent, #667eea); color:white; padding:12px 24px; border-radius:12px; font-size:0.9em; font-weight:600; z-index:999; box-shadow:0 4px 16px rgba(0,0,0,0.2); animation:pulse 0.5s ease;"
...
function applyCharacterStyle(style) {
    ...
    aria.style.filter =
        "drop-shadow(0 0 30px " + style.colors.body + ") brightness(1.5)"
    setTimeout(() => {
    ...
    ariaFeet.forEach(
        foot => (foot.style.transition = "background-color 1s ease"),
    )
...
function addChatMessage(role, text) {
    try {
        ...
        msgWrap.className =
            "chat-msg " +
            (role === "user" ? "user" : role === "aria" ? "aria" : "system")
        ...
        sender.innerText =
            role === "user" ? "You" : role === "aria" ? "Aria" : "System"
...
function arrivalFeedback(waypointKey) {
    try {
        ...
        if (waypointKey && waypoints3D && waypoints3D[waypointKey]) {
            ...
            typeof waypointKey === "string" &&
            waypointKey.includes("%")
        ) {
        ...
        const chosen =
            arrivalMessages[Math.floor(Math.random() * arrivalMessages.length)]
...
function resolveWaypointFromPhrase(phrase) {
    ...
    for (const k in waypoints3D) {
        ...
        if (name.includes(clean) || clean.includes(k.replace("-", " ")))
            return k
    }
...
async function sendChat() {
    ...
    const nlMove = /(?:go to|move to|walk to|goto)\s+([a-z0-9\-\s]+)/i.exec(
        text,
    )
    if (nlMove) {
        ...
        if (candidate) {
            ...
            addChatMessage(
                "aria",
                `I couldn't identify a waypoint for '${targetPhrase}'. Try /waypoints`,
            )
        }
        return
    }
    ...
    if (
        ...
        if (waypoints3D && waypoints3D[waypoint]) {
            ...
            addChatMessage(
                "aria",
                `Unknown waypoint '${waypoint}'. Try /waypoints`,
            )
        }
        return
    }
    ...
    if (text === "/waypoints") {
        const entries = Object.entries(waypoints3D).map(
            ([k, v]) => `${k} — ${v.name}`,
        )
...
async function sendCommand() {
    ...
    try {
        ...
        const ariaY =
            100 - ((ariaRect.bottom - stageRect.top) / stageRect.height) * 100
        ...
        ;["apple", "book", "cup", "ball", "flower"].forEach(objId => {
            ...
            if (obj) {
                ...
                objectPositions[objId] = {
                    x:
                        ((objRect.left - stageRect.left) / stageRect.width) *
                        100,
                    y:
                        100 -
                        ((objRect.bottom - stageRect.top) / stageRect.height) *
                            100,
...
function setPosition(xPercent, yPercent, zDepth = 0, rotateY = 0) {
    ...
    if (distance < 2) {
        log(
            `📍 Already at position (${Math.round(xPercent)}%, ${Math.round(yPercent)}%, Z:${Math.round(zDepth)}px)`,
        )
    ...
    log(
        `🚶 Walking to (${Math.round(xPercent)}%, ${Math.round(yPercent)}%, Z:${Math.round(zDepth)}px, Rot:${Math.round(rotationAngle)}°) - ${walkStyle} style`,
    )
    showFeedback(
        `🚶 3D WALK: X${Math.round(xPercent)}% Y${Math.round(yPercent)}% Z${Math.round(zDepth)}px`,
    )
    ...
    setTimeout(() => {
        ...
        log(
            `✅ Arrived at (${Math.round(xPercent)}%, ${Math.round(yPercent)}%)`,
        )
...
function executeLocalCommand(command) {
    ...
    if (
        cmd.includes("lie down") ||
        cmd.includes("lay down") ||
        cmd.includes("lie ")
    ) {
        applyPose("lie")
        executed = true
    ...
    if (
        cmd.includes("sparkle") ||
        cmd.includes("glitter") ||
        cmd.includes("shimmer") ||
        cmd.includes("shine")
    ) {
    ...
    if (
        cmd.includes("glow") ||
        cmd.includes("glowing") ||
        cmd.includes("radiate") ||
        cmd.includes("illuminate")
    ) {
...
function executeTags(tags) {
    ...
    tags.forEach((tag, index) => {
        ...
        console.log(
            `✅ Parsed tag - Category: ${category}, Action: ${action}, Param: ${param}`,
        )
...
function performGesture(gestureRaw, paramRaw) {
    const gesture = String(gestureRaw || "")
        .toLowerCase()
        .replace(/[-\s]/g, "_")
    ...
    switch (gesture) {
        ...
        case "thumbsup":
            ...
            setTimeout(
                () => moveArm(ariaArmRight, -70, Math.round(duration * 0.2)),
        ...
        case "clap":
            ...
            setTimeout(
                () => resetLimbs(Math.round(duration * 0.25)),
        ...
        case "bow":
            showFeedback("🙇 BOW!")
            if (ariaBody)
                ariaBody.style.transition = `transform ${Math.round(duration * 0.45)}ms ease`
            if (ariaHead)
                ariaHead.style.transition = `transform ${Math.round(duration * 0.45)}ms ease`
            if (ariaBody) ariaBody.style.transform = "scaleY(0.92)"
            if (ariaHead)
                ariaHead.style.transform =
                    "translateX(-50%) translateY(6px) rotate(8deg)"
            setTimeout(
                () => {
        ...
        case "nod":
            showFeedback("🙂 NOD")
            if (ariaHead)
                ariaHead.style.transition = `transform ${Math.round(duration * 0.2)}ms ease`
            if (ariaHead)
        ...
        case "shrug":
            ...
            if (ariaHead)
                ariaHead.style.transform = "translateX(-50%) rotate(6deg)"
            setTimeout(
                () => {
                    resetLimbs(Math.round(duration * 0.35))
                    if (ariaHead) ariaHead.style.transform = "translateX(-50%)"
...
function setEyebrows(state) {
    ...
    const brows = {
        happy: {
            l: "rotate(-8deg) translateY(-3px)",
            r: "rotate(8deg) translateY(-3px)",
        },
        smile: {
            l: "rotate(-8deg) translateY(-3px)",
            r: "rotate(8deg) translateY(-3px)",
        ...
        sad: {
            l: "rotate(5deg) translateY(1px)",
            r: "rotate(-5deg) translateY(1px)",
        },
        angry: {
            l: "rotate(-14deg) translateY(4px)",
            r: "rotate(14deg) translateY(4px)",
        },
        surprised: {
            l: "rotate(-3deg) translateY(-6px)",
            r: "rotate(3deg) translateY(-6px)",
        },
        confused: {
            l: "rotate(8deg) translateY(-1px)",
            r: "rotate(-3deg) translateY(-2px)",
        },
        thinking: {
            l: "rotate(-4deg) translateY(-3px)",
            r: "rotate(7deg) translateY(1px)",
        },
...
function changeExpression(emotion) {
    ...
    if (emoji && stage && aria) {
        ...
        requestAnimationFrame(() => {
            el.style.transform = "translateY(-30px) scale(1.3)"
            el.style.opacity = "0"
        })
...
function startIdleAnimation() {
    ...
    idleAnimationInterval = setInterval(() => {
        ...
        if (ariaBody) {
            ...
            setTimeout(() => {
                ariaBody.style.transform = "scaleY(1) scaleX(1)"
            }, 2500)
        }
...
function handleLimbTag(partRaw, paramRaw) {
    ...
    if (typeof value === "number") {
        ...
        if (isArm) moveArm(targetEl, angle, duration)
        else moveLeg(targetEl, angle, duration)
        setTimeout(
            () => {
                isPerformingAction = false
            },
    ...
    switch (action) {
        case "raise":
        case "up":
            if (isArm) moveArm(targetEl, -90, duration)
            else moveLeg(targetEl, -30, duration)
            break
        case "lower":
        case "down":
            if (isArm) moveArm(targetEl, 0, duration)
            else moveLeg(targetEl, 0, duration)
            break
        case "forward":
            if (isArm) moveArm(targetEl, -45, duration)
            else moveLeg(targetEl, 30, duration)
            break
        case "back":
        case "backward":
...
function startIdleSparkles() {
    ...
    _idleSparkleInterval = setInterval(() => {
        ...
        if (
            aria.classList.contains("walking") ||
            aria.classList.contains("dancing") ||
            aria.classList.contains("running")
        )
            return
        ...
        const stageRect = stage.getBoundingClientRect()
        const cx =
            ((rect.left + rect.width / 2 - stageRect.left) / stageRect.width) *
            100
        const cy =
            ((rect.top + rect.height * 0.3 - stageRect.top) /
                stageRect.height) *
            100
...
function pickUpObject(objectId) {
    ...
    if (characterState.heldObject) {
        showFeedback(
            "⚠️ Already holding " +
                characterState.heldObject.toUpperCase() +
                "!",
        )
        return false
    }
...
function dropObject(name) {
    ...
    const obj =
        characterState.heldObjectElement || document.getElementById(targetName)
    if (!obj) {
    ...
    const dropBottom =
        ((stageRect.bottom - ariaRect.bottom) / stageRect.height) * 100
    ...
    const isOnTable =
        Math.abs(finalX - tableX) < 20 && Math.abs(finalY - 35) < 20
    sendObjectUpdate("update", obj.id, {
        position: { x: finalX, y: finalY },
        state: isOnTable ? "on_table" : "on_stage",
    }).catch(() => {})
...
function positionHeldObject() {
    ...
    const objLeft =
        ((ariaRect.left - stageRect.left + ariaRect.width * 0.6) /
            stageRect.width) *
        100
    const objBottom =
        ((stageRect.bottom - ariaRect.top + 20) / stageRect.height) * 100
...
function initializeObjectInteractions() {
    ...
    objects.forEach(obj => {
        ...
        obj.addEventListener("mousedown", e => {
            ...
            startBottom =
                ((stageRect.bottom - rect.bottom) / stageRect.height) * 100
        ...
        document.addEventListener("mousemove", e => {
            ...
            obj.style.left =
                Math.max(0, Math.min(100, startLeft + deltaX)) + "%"
            obj.style.bottom =
                Math.max(0, Math.min(100, startBottom + deltaY)) + "%"
        })
        ...
        document.addEventListener("mouseup", () => {
            if (isDragging) {
                ...
                const state = obj.classList.contains("held")
                    ? "held"
                    : "on_table"
                sendObjectUpdate("update", obj.id, {
                    position: pos,
...
function addObject(objectName, emoji) {
    ...
    sendObjectUpdate("add", objectName, {
        position: objectPositionFromElement(newObj),
        state: "on_stage",
        emoji: emoji,
    }).catch(() => {})
...
;(function initEyeTracking() {
    ...
    document.addEventListener("mousemove", function (e) {
        eyes.forEach(function (eye) {
            ...
            const dist = Math.min(
                3,
                Math.hypot(e.clientX - cx, e.clientY - cy) / 50,
            )
...
window.ariaTest = {
    limb: (part, actionOrAngle, duration) =>
        handleLimbTag(
            part,
            typeof actionOrAngle === "number"
                ? `${actionOrAngle},${duration || 500}`
                : `${actionOrAngle || ""}${duration ? "," + duration : ""}`,
        ),
