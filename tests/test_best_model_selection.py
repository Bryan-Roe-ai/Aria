...
def test_best_model_selection():
    ...
    def _select_best_model(metrics, ranking_metric):
        ...
        if ranking_metric in ('perplexity_improvement', 'combined_improvement'):
            if ranking_metric == 'combined_improvement':
                for m in filtered:
                    m['combined_improvement'] = 0.7 * \
                        (improvement(m) or 0.0)+0.3*(diversity(m) or 0.0)
                filtered = [m for m in filtered if m.get(
                    'combined_improvement') is not None]

                def key(x): return x.get('combined_improvement')
            else:
                filtered = [m for m in filtered if improvement(m) is not None]
                def key(x): return improvement(x)
    ...
    metrics = [
        {"model": "phi", "run_id": "r1", "pre_perplexity": 30.0, "post_perplexity": 20.0,
            "perplexity_improvement": (30-20)/30, "diversity_avg": 0.5},
        {"model": "tinyllama", "run_id": "r2", "pre_perplexity": 10.0,
            "post_perplexity": 9.0, "perplexity_improvement": (10-9)/10, "diversity_avg": 0.9}
    ]
