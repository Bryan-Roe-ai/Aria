...
def test_evaluate_small(tmp_path: Path, samples_per_class):
    ...
    rc = train.main(['--dataset', str(ds), '--epochs', '1',
                    '--batch-size', '2', '--out-dir', str(out)])
    assert rc == 0
    ...
    # run evaluation
    rc2 = eval_mod.run_eval(['--checkpoint', str(ck), '--dataset', str(ds), '--out-dir',
                            str(out / 'eval_out'), '--img-size', '32', '--show-examples', '2'])
    assert rc2 == 0
