...
;(function (DAL) {
    ...
    ;((DAL.ACCELEROMETER_IMU_DATA_VALID = 2),
        (DAL.ACCELEROMETER_EVT_DATA_UPDATE = 1),
        (DAL.ACCELEROMETER_EVT_NONE = 0),
        (DAL.ACCELEROMETER_EVT_TILT_UP = 1),
        (DAL.ACCELEROMETER_EVT_TILT_DOWN = 2),
        (DAL.ACCELEROMETER_EVT_TILT_LEFT = 3),
        (DAL.ACCELEROMETER_EVT_TILT_RIGHT = 4),
        (DAL.ACCELEROMETER_EVT_FACE_UP = 5),
        (DAL.ACCELEROMETER_EVT_FACE_DOWN = 6),
        (DAL.ACCELEROMETER_EVT_FREEFALL = 7),
        (DAL.ACCELEROMETER_EVT_3G = 8),
        (DAL.ACCELEROMETER_EVT_6G = 9),
        (DAL.ACCELEROMETER_EVT_8G = 10),
        (DAL.ACCELEROMETER_EVT_SHAKE = 11),
        (DAL.ACCELEROMETER_REST_TOLERANCE = 200),
        (DAL.ACCELEROMETER_TILT_TOLERANCE = 200),
        (DAL.ACCELEROMETER_FREEFALL_TOLERANCE = 400),
        (DAL.ACCELEROMETER_SHAKE_TOLERANCE = 400),
        (DAL.ACCELEROMETER_3G_TOLERANCE = 3072),
        (DAL.ACCELEROMETER_6G_TOLERANCE = 6144),
        (DAL.ACCELEROMETER_8G_TOLERANCE = 8192),
        (DAL.ACCELEROMETER_GESTURE_DAMPING = 5),
        (DAL.ACCELEROMETER_SHAKE_DAMPING = 10),
        (DAL.ACCELEROMETER_SHAKE_RTX = 30),
        (DAL.ACCELEROMETER_SHAKE_COUNT_THRESHOLD = 4),
        (DAL.DEVICE_ID_GESTURE = 13),
        (DAL.DEVICE_ID_ACCELEROMETER = 5),
        (DAL.SENSOR_THRESHOLD_LOW = 1),
        (DAL.SENSOR_THRESHOLD_HIGH = 2),
        (DAL.LEVEL_THRESHOLD_LOW = 1),
        (DAL.LEVEL_THRESHOLD_HIGH = 2),
        (DAL.DEVICE_ID_THERMOMETER = 8),
        (DAL.DEVICE_ID_LIGHT_SENSOR = 17),
        (DAL.DEVICE_ID_RADIO = 9),
        (DAL.DEVICE_RADIO_EVT_DATAGRAM = 1),
        (DAL.DEVICE_ID_MICROPHONE = 3001))
})(DAL || (DAL = {}))
var pxsim
;(function (pxsim) {
    let PlayerNumber
        ...
        if (g) {
            for (let i = 0; i < g.length; i++) {
                ...
                if (gamepad && gamepad.buttons && gamepad.buttons.length) {
                    ...
                    if (ctrl.player == 1) {
                        //Support reset for first player only
...
;(function (pxsim) {
    ...
    window.addEventListener("DOMContentLoaded", () => {
        ...
        if (pxsim.simButtonsHidden) hideSimButtons()
        if (!!searchParams.get("noExtraPadding")) noExtraPadding()
    })
    if (hasNavigator()) {
    ...
    function registerPointerEvents(sendMessages) {
        ...
        const reporter = event => {
            ...
            const x =
                ((event.clientX - rect.left - xOffset) / actualWidth) *
                canvas.width
            const y =
                ((event.clientY - rect.top - yOffset) / actualHeight) *
                canvas.height
    ...
    pxsim.initCurrentRuntime = msg => {
        ...
        if (!forcedUpdateLoop) {
            ...
            window.addEventListener("message", ev => {
                var _a
                if (
                    ev.data.button !== undefined &&
                    ev.data.type !== "multiplayer"
                ) {
                    let key
                    ...
                    const b = board()
                    if (b) b.setButton(key, ev.data.pressed)
                }
                if (ev.data.context !== undefined) {
                    if (
                        ev.data.context == "client" ||
                ...
                if (ev.data.type == "setactiveplayer") {
                    ...
                    pxsim.theme.setSimThemeColor(
                        ev.data.part,
                        (_a = ev.data.color) === null || _a === void 0
                            ? void 0
                            : _a.replace("#", ""),
                    )
                }
    ...
    class Board extends pxsim.BaseBoard {
        constructor() {
            ...
            this.accelerometerState = new pxsim.AccelerometerState(
                pxsim.runtime,
            )
            this.microphoneState = new pxsim.MicrophoneState(
                DAL.DEVICE_ID_MICROPHONE,
                0,
                255,
                50,
                120,
            ...
            ;[
                ...
                for (let i = 0; i < pinp.count; ++i) {
                    ...
                    if (pxsim.getConfig(id) == null) pxsim.setConfig(id, id)
                }
            })
            ...
            this.lightSensorState = new pxsim.AnalogSensorState(
                DAL.DEVICE_ID_LIGHT_SENSOR,
            )
            this.thermometerState = new pxsim.AnalogSensorState(
            ...
            throttleAnimation(
                cb => (this.screenState.onChange = cb),
                () => this.gameplayer.draw(this.screenState),
            )
            this.activePlayer = undefined
        }
        setActivePlayer(playerNumber, theme) {
            if (this.multiplayerState && this.multiplayerState.origin) return
            const playerThemes = [undefined, "p1", "p2", "p3", "p4"]
        ...
        setButton(which, isPressed) {
            ...
            if (inMultiplayerSession && which > pxsim.Key.Menu) return
            if (!which) return
            let playerOffset = 0
            if (
        ...
        handleKeyEvent(key, isPressed, playerOffset = 0) {
            ...
            switch (key) {
                ...
                case pxsim.Key.Screenshot:
                    ...
                    return
                case pxsim.Key.Gif:
                    if (!isPressed) pxsim.Runtime.requestToggleRecording()
                    break
            ...
            this.bus.queue(
                isPressed ? INTERNAL_KEY_DOWN : INTERNAL_KEY_UP,
                gameKey,
            )
            // no 'any' for p2-4
            if (!playerOffset) {
        ...
        async initAsync(msg) {
            ...
            if (isFirstRunSafari && !safariEnablePromise) {
                const safariWarning =
                    document.getElementById("safari-enable-game")
                if (
                    isSafari() &&
                    msg.options &&
                    msg.options.mpRole === "server"
                ) {
        ...
        neopixelState(pinId) {
            if (pinId === undefined) pinId = this.edgeConnectorState.pins[0].id
...
function isSafari() {
    ...
    return (
        ...
        !isEdge() &&
        !!navigator &&
        /(Macintosh|Safari|iPod|iPhone|iPad)/i.test(navigator.userAgent)
    )
}
;(function (pxsim) {
    var pxtcore
...
;(function (pxsim) {
    ...
    ;(function (Keyboard) {
        ...
        function getLocalizedInstructions() {
            return pxsim.localization.lf(
                "Type your response using your keyboard and hit ENTER",
            )
        }
...
;(function (pxsim) {
    ...
    ;(function (theme_1) {
        ...
        function setSimThemeColor(part, color) {
            if (
                !part ||
                !(color == undefined || /^(#|0x)?[0-9A-F]{6}$/i.test(color))
            )
                return
            if (
    ...
    ;(function (visuals) {
        class GameButtons {
            ...
            constructor(container) {
                ...
                this.logEvents = () => {
                    if (
                        Object.keys(this.buttonPressCount).some(
                            x => !!this.buttonPressCount[x],
                        )
                    ) {
                        // tickEvent("shareExperiment.play.buttonPress", this.buttonPressCount);
                        Object.keys(this.buttonPressCount).forEach(
                            k => (this.buttonPressCount[k] = 0),
                        )
                    }
                }
                ...
                this.parent =
                    container ||
                    document.getElementById("game-buttons-container")
                this.aButton = this.parent
                    .getElementsByClassName("button-a")
                    .item(0)
                this.aLabel = this.parent
                    .getElementsByClassName("label-a")
                    .item(0)
                this.bButton = this.parent
                    .getElementsByClassName("button-b")
                    .item(0)
                this.bLabel = this.parent
                    .getElementsByClassName("label-b")
                    .item(0)
                this.dragSurface = this.parent
                    .getElementsByClassName("game-button-svg")
                    .item(0)
            ...
            pointIsWithinCircle(x, y, circle) {
                ...
                const distance = Math.sqrt(
                    Math.pow(x - (bounds.left + radius), 2) +
            ...
            updateButtonGesture(x, y) {
                this.setButtonState(
                    pxsim.Key.A,
                    this.pointIsWithinCircle(x, y, this.aButton),
                )
                this.setButtonState(
                    pxsim.Key.B,
                    this.pointIsWithinCircle(x, y, this.bButton),
                )
            ...
            setButtonState(button, pressed, quiet = false) {
                ...
                if (circle && label) {
                    circle.setAttribute(
                        "fill",
                        pressed
                            ? "var(--sim-background-color)"
                            : "var(--sim-button-fill)",
                    )
                    label.setAttribute(
                        "fill",
                        pressed ? "var(--sim-button-fill)" : "",
                    )
                }
                if (!quiet) {
                    if (pressed) {
                    ...
                    } else visuals.releaseButton(button)
                }
            }
            bindEvents(surface) {
                if (!surface) return
            ...
            bindEvent(element, event, callback) {
                this.bindings.push([element, event, callback])
    ...
    ;(function (visuals) {
        class GamePlayer {
            constructor(scaleFactor = 1) {
                ...
                this.menu =
                    document.getElementsByClassName("game-menu-button")[0]
                this.reset =
            ...
            draw(state) {
                ...
                if (this.scaleFactor === 1) {
                    if (
                        state.width !== this.screen.width ||
                        state.height !== this.screen.height
                    ) {
                        this.screen.width = state.width
                        this.screen.height = state.height
                        this.screen.className = ""
                    }
                    let img = context.getImageData(
                        0,
                        0,
                        state.width,
                        state.height,
                    ...
                    if (
                        state.width !== this.screen.width * this.scaleFactor ||
                        state.height !== this.screen.height * this.scaleFactor
                    ) {
                    ...
                    for (let x = 0; x < state.width; x++) {
                        for (let y = 0; y < state.height; y++) {
                            context.fillStyle =
                                this.palette[
                                    state.lastImage.data[x + y * state.width] &
                                        mask
                                ]
                            context.fillRect(
                                x * this.scaleFactor,
            ...
            paletteDidChange(state) {
                if (
                    !this.cachedPalette ||
                    this.cachedPalette.length != state.palette.length
                )
                    return true
                for (let i = 0; i < this.cachedPalette.length; i++) {
                    if (this.cachedPalette[i] != state.palette[i]) return true
                }
                return false
            }
    ...
    ;(function (visuals) {
        ...
        class Joystick {
            ...
            constructor(container) {
                ...
                this.dPadUp = this.parent
                    .getElementsByClassName("dpad-up")
                    .item(0)
                this.dPadDown = this.parent
                    .getElementsByClassName("dpad-down")
                    .item(0)
                this.dPadLeft = this.parent
                    .getElementsByClassName("dpad-left")
                    .item(0)
                this.dPadRight = this.parent
                    .getElementsByClassName("dpad-right")
                    .item(0)
                this.joystickHandle = this.parent
                    .getElementsByClassName("joystick-handle")
                    .item(0)
                this.dragSurface = this.parent
                    .getElementsByClassName("game-joystick-svg")
                    .item(0)
                this.bindEvents(this.dragSurface)
            }
            dispose() {
            ...
            updateJoystickDrag(x, y) {
                if (this.joystickHandle) {
                    ...
                    const dx =
                        (x - bounds.left) * (SVG_WIDTH / bounds.width) -
                        HALF_WIDTH
                    const dy =
                        (y - bounds.top) * (SVG_WIDTH / bounds.height) -
                        HALF_WIDTH
                    ...
                    this.setHandlePosition(
                        HALF_WIDTH + distance * Math.cos(angle),
                        HALF_WIDTH + distance * Math.sin(angle),
                    )
                }
            }
            startAnimation() {
                ...
                if (this.joystickHandle) {
                    ...
                    if (!animation) {
                        if (this.getHandleDistance() < 5) {
                            ...
                            const angle = this.getHandleAngle()
                            const octet =
                                (5 + Math.floor(angle / (Math.PI / 4) - 0.5)) %
                                8
                            ...
                            if (down) visuals.pressButton(pxsim.Key.Down)
                            else visuals.releaseButton(pxsim.Key.Down)
                            if (up) visuals.pressButton(pxsim.Key.Up)
                            else visuals.releaseButton(pxsim.Key.Up)
                            if (left) visuals.pressButton(pxsim.Key.Left)
                            else visuals.releaseButton(pxsim.Key.Left)
                            if (right) visuals.pressButton(pxsim.Key.Right)
                            else visuals.releaseButton(pxsim.Key.Right)
                        }
                    }
                }
            }
            getHandleAngle() {
                return Math.atan2(
                    this.handleY - HALF_WIDTH,
                    this.handleX - HALF_WIDTH,
                )
            }
            getHandleDistance() {
                return Math.sqrt(
                    Math.pow(this.handleX - HALF_WIDTH, 2) +
                        Math.pow(this.handleY - HALF_WIDTH, 2),
                )
            }
            clearButtonPresses() {
...
;(function (pxsim) {
    ...
    ;(function (RotaryEncoderMethods) {
        ...
        (RotaryEncoderMethods =
            pxsim.RotaryEncoderMethods || (pxsim.RotaryEncoderMethods = {})),
    )
})(pxsim || (pxsim = {}))
var pxsim
;(function (pxsim) {
...
;(function (pxsim) {
    ...
    ;(function (ImageMethods) {
        ...
        function fillRect(img, x, y, w, h, c) {
            ...
            c = img.color(c)
            while (h-- > 0) {
                for (let i = 0; i < w; ++i) img.data[p++] = c
                p += d
            }
        }
        ...
        function equals(img, other) {
            if (
                !other ||
                img._bpp != other._bpp ||
                img._width != other._width ||
                img._height != other._height
            ) {
                return false
            }
        ...
        function copyFrom(img, from) {
            if (
                img._width != from._width ||
                img._height != from._height ||
                img._bpp != from._bpp
            )
                return
        ...
        function scroll(img, dx, dy) {
            ...
            if (dx != 0) {
                ...
                dy = -dy
                if (dy < img._height) img.data.copyWithin(0, dy * img._width)
                else dy = img._height
                img.data.fill(0, (img._height - dy) * img._width)
            } else if (dy > 0) {
                if (dy < img._height) img.data.copyWithin(dy * img._width, 0)
                else dy = img._height
        ...
        function drawImageCore(img, from, x, y, clear, check) {
            ...
            if (clear) fillRect(img, x, y, from._width, from._height, 0)
            else if (!check) img.makeWritable()
        ...
        function drawLine(img, x0, y0, x1, y1, c) {
            ...
            let h = y1 - y0
            if (h == 0) {
                if (w == 0) setPixel(img, x0, y0, c)
                else fillRect(img, x0, y0, w + 1, 1, c)
                return
            }
            if (w == 0) {
                if (h > 0) fillRect(img, x0, y0, 1, h + 1, c)
                else fillRect(img, x0, y1, 1, -h + 1, c)
                return
            }
            if (x1 < 0 || x0 >= img._width) return
            if (x0 < 0) {
                y0 -= ((h * x0) / w) | 0
                x0 = 0
            }
            ...
            if (h < 0) {
                h = -h
                if (h < w) drawLineLow(img, x0, y0, x1, y1, c)
                else drawLineHigh(img, x1, y1, x0, y0, c)
            } else {
                if (h < w) drawLineLow(img, x0, y0, x1, y1, c)
                else drawLineHigh(img, x0, y0, x1, y1, c)
            }
        }
        ImageMethods.drawLine = drawLine
        function drawIcon(img, icon, x, y, color) {
            const src = icon.data
            if (!pxsim.image.isValidImage(icon)) return
            if (src[1] != 1) return // only mono
        ...
        function fillPolygon4(img, x0, y0, x1, y1, x2, y2, x3, y3, c) {
            const lines = [
                ...
                x2 < x3
                    ? initYRangeGenerator(x2, y2, x3, y3)
                    : initYRangeGenerator(x3, y3, x2, y2),
                x0 < x3
                    ? initYRangeGenerator(x0, y0, x3, y3)
        ...
        function blitRow(img, x, y, from, fromX, fromH) {
            ...
            let endY = y + fromH
            if (endY > img._height) endY = img._height
            if (y < 0) {
                fy += -y * stepFY
                y = 0
            }
        ...
        function blit(dst, src, args) {
            ...
            if (!check) dst.makeWritable()
            for (
                let yDstCur = yDstStart, ySrcCur = ySrcStart;
                yDstCur < yDstEnd && ySrcCur < ySrcEnd;
                ++yDstCur, ySrcCur += ySrcStep
            ) {
                const ySrcCurI = ySrcCur >> 16
                for (
                    let xDstCur = xDstStart, xSrcCur = xSrcStart;
                ...
                for (let y = 0; y < shearArgs.scaledHeight; y += FX_ONE) {
                    for (let x = 0; x < shearArgs.scaledWidth; x += FX_ONE) {
                        let color = getPixel(
                            ...
                            fxToInt(
                                fxDiv(
                                    shearArgs.scaledWidth - x - FX_ONE,
                                    shearArgs.sx,
                                ),
                            ),
                            fxToInt(
                                fxDiv(
                                    shearArgs.scaledHeight - y - FX_ONE,
                                    shearArgs.sy,
                                ),
                            ),
                        )
                        ...
                        setPixel(
                            dst,
                            xDst + fxToInt(shearedX - shearArgs.minX),
        ...
        ImageMethods._checkOverlapsScaledRotatedImage =
            _checkOverlapsScaledRotatedImage
                ...
                for (let y = 0; y < srcArgs.scaledHeight; y += FX_ONE) {
                    for (let x = 0; x < srcArgs.scaledWidth; x += FX_ONE) {
                        let color = getPixel(
                            ...
                            fxToInt(
                                fxDiv(
                                    srcArgs.scaledWidth - x - FX_ONE,
                                    srcArgs.sx,
                                ),
                            ),
                            fxToInt(
                                fxDiv(
                                    srcArgs.scaledHeight - y - FX_ONE,
                                    srcArgs.sy,
                                ),
                            ),
                        )
                        ...
                        REVERSE_SHEAR(
                            screenX + dstArgs.minX,
                            screenY + dstArgs.minY,
                            dstArgs.xShear,
                            dstArgs.yShear,
                        )
                        if (dstArgs.flip) {
        ...
        ImageMethods._checkOverlapsTwoScaledRotatedImages =
            _checkOverlapsTwoScaledRotatedImages
...
;(function (pxsim) {
    ...
    ;(function (image) {
        function byteHeight(h, bpp) {
            if (bpp == 1) return (h * bpp + 7) >> 3
            else return ((h * bpp + 31) >> 5) << 2
        }
        image.byteHeight = byteHeight
        function isLegacyImage(buf) {
            if (!buf || buf.data.length < 5) return false
            if (buf.data[0] != 0xe1 && buf.data[0] != 0xe4) return false
            const bpp = buf.data[0] & 0xf
            const sz = buf.data[1] * byteHeight(buf.data[2], bpp)
            if (4 + sz != buf.data.length) return false
            return true
        }
        function bufW(data) {
        ...
        function isValidImage(buf) {
            if (!buf || buf.data.length < 5) return false
            if (buf.data[0] != 0x87) return false
            if (buf.data[1] != 1 && buf.data[1] != 4) return false
            const bpp = buf.data[1]
            const sz = bufW(buf.data) * byteHeight(bufH(buf.data), bpp)
            if (8 + sz != buf.data.length) return false
            return true
        }
        ...
        function create(w, h) {
            w |= 0
            h |= 0
            if (w < 0 || h < 0 || w > 2000 || h > 2000) return undefined
...
;(function (pxsim) {
    ...
    ;(function (pxtcore) {
        ...
        function setScreenBrightness(b) {
            ...
            const state = pxsim.getScreenState()
            if (state) state.setScreenBrightness(b)
        }
    ...
    ;(function (music) {
        ...
        async function _createSequencer() {
            ...
            res.sequencer.addEventListener("state-change", () => {
                pxsim
                    .board()
                    .bus.queue(music.SEQUENCER_STATE_CHANGE_MESSAGE, res.id)
            })
        ...
        function _sequencerState(id) {
            var _a
            return (_a = lookupSequencer(id)) === null || _a === void 0
                ? void 0
                : _a.state()
        }
        ...
        function _sequencerPlaySong(id, song, loop) {
            ...
            ;(_a = lookupSequencer(id)) === null || _a === void 0
                ? void 0
                : _a.start(decoded, loop)
        }
        ...
        function _sequencerSetTrackVolume(id, trackIndex, volume) {
            ...
            ;(_a = lookupSequencer(id)) === null || _a === void 0
                ? void 0
                : _a.setTrackVolume(trackIndex, volume)
        }
        music._sequencerSetTrackVolume = _sequencerSetTrackVolume
        function _sequencerSetDrumTrackVolume(
            id,
            trackIndex,
            drumIndex,
            volume,
            ...
            ;(_a = lookupSequencer(id)) === null || _a === void 0
                ? void 0
                : _a.setDrumTrackVolume(trackIndex, drumIndex, volume)
        }
        ...
        function lookupSequencer(id) {
            for (const seq of sequencers)
                if (seq.id === id) return seq.sequencer
            return undefined
        }
    })((music = pxsim.music || (pxsim.music = {})))
})(pxsim || (pxsim = {}))
...
;(function (pxsim) {
    ...
    ;(function (settings) {
        ...
        settings._get = _get
        function _userClean() {
            for (let k of userKeys()) pxsim.board().setStoredState(k, null)
            computeSize()
            // if system keys take more than 25% of space, delete everything
            if (currSize > MAX_SIZE / 4) {
                for (let k of allKeys()) pxsim.board().setStoredState(k, null)
                computeSize()
            }
        }
        ...
        function _list(prefix) {
            ...
            for (let k of prefix[0] == "#" ? allKeys() : userKeys()) {
                ...
                if (n.slice(0, prefix.length) != prefix) continue
                r.push(n)
            }
            return r
        }
...
;(function (pxsim) {
    ...
    ;(function (keymap) {
        ...
        function _setPlayerKeys(
            player, // player number is 1-based
            up,
            down,
            left,
            right,
            ...
            pxsim
                .getKeymapState()
                .setPlayerKeys(player, up, down, left, right, A, B)
        }
...
;(function (pxsim) {
    ...
    class KeymapState {
        ...
        setPlayerKeys(
            ...
            const filtered = keyCodes.filter(keyCode =>
                reservedKeyCodes.includes(keyCode),
            )
        ...
        getKey(keyCode) {
            return keyCode
                ? this.keymap[keyCode] || this.altmap[keyCode] || Key.None
                : Key.None
        }
...
;(function (pxsim) {
    ...
    ;(function (multiplayer) {
        const throttledImgPost = pxsim.U.throttle(
            msg => {
        ...
        function postImage(im) {
            if (pxsim.getMultiplayerState().origin !== "server") return
        ...
        function postIcon(iconType, slot, im) {
            ...
            const asBuf =
                im && im.data.some(pixel => pixel != 0)
                    ? pxsim.image.toBuffer(im)
                    : undefined
            const sb = pxsim.board()
            const screenState = sb && sb.screenState
...
;(function (pxsim) {
    ...
    class MultiplayerState {
        ...
        init(origin) {
            this.origin = origin
            pxsim.runtime.board.addMessageListener(msg =>
            ...
            if (isImageMessage(msg)) {
                ...
                if (this.origin === "client") {
                    if (msg.instruction === "playinstructions") {
                        pxsim.AudioContextManager.playInstructionsAsync(
                            msg.soundbuf,
                        )
...
;(function (pxsim) {
    ...
    ;(function (input) {
        ...
        function rotation(kind) {
            ...
            let pitch = Math.atan(
                -x / (y * Math.sin(roll) + z * Math.cos(roll)),
            )
            let r = 0
            switch (kind) {
                case 0:
        ...
        function acceleration(dimension) {
            ...
            switch (dimension) {
                ...
                default:
                    return Math.floor(
                        Math.sqrt(acc.instantaneousAccelerationSquared()),
                    )
            }
        }
...
;(function (pxsim) {
    ...
    ;(function (MicroBitCoordinateSystem) {
        ...
        MicroBitCoordinateSystem[
            (MicroBitCoordinateSystem["SIMPLE_CARTESIAN"] = 1)
        ] = "SIMPLE_CARTESIAN"
        MicroBitCoordinateSystem[
            (MicroBitCoordinateSystem["NORTH_EAST_DOWN"] = 2)
        ] = "NORTH_EAST_DOWN"
    })(
        (MicroBitCoordinateSystem =
            pxsim.MicroBitCoordinateSystem ||
            (pxsim.MicroBitCoordinateSystem = {})),
    )
    class Accelerometer {
        constructor(runtime) {
            ...
            this.shake = {
                ...
                shaken: 0,
                timer: 0,
            } // State information needed to detect shake events.
            this.isActive = false
            this.sampleRange = 2
        ...
        instantaneousAccelerationSquared() {
            ...
            return (
                this.sample.x * this.sample.x +
                this.sample.y * this.sample.y +
                this.sample.z * this.sample.z
            )
        }
        /**
        ...
        instantaneousPosture() {
            ...
            // has been shaken.
            if (
                (this.getX() < -DAL.ACCELEROMETER_SHAKE_TOLERANCE &&
                    this.shake.x) ||
                (this.getX() > DAL.ACCELEROMETER_SHAKE_TOLERANCE &&
                    !this.shake.x)
            ) {
            ...
            if (
                (this.getY() < -DAL.ACCELEROMETER_SHAKE_TOLERANCE &&
                    this.shake.y) ||
                (this.getY() > DAL.ACCELEROMETER_SHAKE_TOLERANCE &&
                    !this.shake.y)
            ) {
            ...
            if (
                (this.getZ() < -DAL.ACCELEROMETER_SHAKE_TOLERANCE &&
                    this.shake.z) ||
                (this.getZ() > DAL.ACCELEROMETER_SHAKE_TOLERANCE &&
                    !this.shake.z)
            ) {
            ...
            if (
                shakeDetected &&
                this.shake.count < DAL.ACCELEROMETER_SHAKE_COUNT_THRESHOLD &&
                ++this.shake.count == DAL.ACCELEROMETER_SHAKE_COUNT_THRESHOLD
            )
                this.shake.shaken = 1
            if (++this.shake.timer >= DAL.ACCELEROMETER_SHAKE_DAMPING) {
                ...
                if (this.shake.count > 0) {
                    if (--this.shake.count == 0) this.shake.shaken = 0
                }
            }
            if (this.shake.shaken) return DAL.ACCELEROMETER_EVT_SHAKE
            let sq = n => n * n
        ...
        updateGesture() {
            ...
            if (g == this.currentGesture) {
                if (this.sigma < DAL.ACCELEROMETER_GESTURE_DAMPING) this.sigma++
            } else {
            ...
            if (
                ...
                this.sigma >= DAL.ACCELEROMETER_GESTURE_DAMPING
            ) {
        ...
        recalculatePitchRoll() {
            ...
            this.pitch = Math.atan(
                -x / (y * Math.sin(this.roll) + z * Math.cos(this.roll)),
            )
        }
    }
                ...
                ev => {
                    ...
                    if (!this.tiltDecayer) {
                        this.tiltDecayer = setInterval(() => {
                            ...
                            accx =
                                Math.floor(Math.abs(accx) * 0.85) *
                                (accx > 0 ? 1 : -1)
                            let accy = this.accelerometer.getY()
                            accy =
                                Math.floor(Math.abs(accy) * 0.85) *
                                (accy > 0 ? 1 : -1)
                            let accz = -Math.sqrt(
                                Math.max(
                                    0,
                                    1023 * 1023 - accx * accx - accy * accy,
                                ),
                            )
...
;(function (pxsim) {
    ...
    ;(function (input) {
        function temperature(unit) {
            ...
            return unit == pxsim.TemperatureUnit.Celsius
                ? deg
                : ((deg * 18) / 10 + 32) >> 0
        }
        input.temperature = temperature
        function onTemperatureConditionChanged(
            condition,
            temperature,
            unit,
            body,
            ...
            pxsim.setThermometerUnit(unit)
            const t =
                unit == pxsim.TemperatureUnit.Celsius
                    ? temperature
...
;(function (pxsim) {
    ...
    ;(function (radio) {
        ...
        function readRawPacket() {
            ...
            for (let i = 0; i < buf.length; ++i) rbuf.data[i] = buf[i]
            // append RSSI
            pxsim.BufferMethods.setNumber(
                rbuf,
                pxsim.BufferMethods.NumberFormat.Int32LE,
                n,
                packet.rssi,
            )
            return rbuf
...
;(function (pxsim) {
    ...
    class RadioDatagram {
        ...
        queue(packet) {
            if (this.datagram.length < 4) this.datagram.push(packet)
            pxsim.runtime.board.bus.queue(
                this.dal.ID_RADIO,
                this.dal.RADIO_EVT_DATAGRAM,
            )
        }
        send(payload) {
            ...
            pxsim.Runtime.postMessage({
                ...
                broadcast: true,
                rssi: this._rssi || -75,
                serial: state.transmitSerialNumber
                    ? pxsim.control.deviceSerialNumber()
                    : 0,
...
;(function (pxsim) {
    ...
    ;(function (control) {
        ...
        function deviceName() {
            let b = pxsim.board()
            return b && b.id ? b.id.slice(0, 4) : "abcd"
        }
        ...
        function deviceLongSerialNumber() {
            ...
            pxsim.BufferMethods.setNumber(
                b,
                pxsim.BufferMethods.NumberFormat.UInt32LE,
                0,
                deviceSerialNumber(),
            )
            return b
    ...
    class ControlMessageState {
        ...
        enqueue(message) {
            ...
            this.board.bus.queue(
                pxsim.CONTROL_MESSAGE_EVT_ID,
                pxsim.CONTROL_MESSAGE_RECEIVED,
            )
        }
        peek() {
...
;(function (pxsim) {
        ...
        setValue(value) {
            ...
            if (
                ...
                b.bus.queue(
                    this.id,
                    this.value > 0
                        ? DAL.DEVICE_PIN_EVT_RISE
                        : DAL.DEVICE_PIN_EVT_FALL,
                )
        }
        digitalReadPin() {
    ...
    class EdgeConnectorState {
        constructor(props) {
            ...
            this._spis = []
            this._serials = []
            this.pins = props.pins.map(id =>
                id != undefined ? new Pin(id) : null,
            )
        }
        getPin(id) {
        ...
        createI2C(sda, scl) {
            ...
            if (!ser) this._i2cs.push((ser = new I2C(sda, scl)))
            return ser
        }
        createSPI(mosi, miso, sck) {
            let ser = this._spis.filter(
                s => s.mosi == mosi && s.miso == miso && s.sck == sck,
            )[0]
            if (!ser) this._spis.push((ser = new SPI(mosi, miso, sck)))
            return ser
        }
        createSerialDevice(tx, rx, id) {
            let ser = this._serials.filter(s => s.tx == tx && s.rx == rx)[0]
            if (!ser) this._serials.push((ser = new SerialDevice(tx, rx, id)))
            return ser
        }
    }
    ...
    ;(function (DigitalInOutPinMethods) {
        ...
        function onPulsed(name, high, body) {
            ...
            onEvent(
                name,
                high
                    ? DAL.DEVICE_PIN_EVT_PULSE_HI
                    : DAL.DEVICE_PIN_EVT_PULSE_LO,
                body,
            )
        }
...
;(function (pxsim) {
    ...
    ;(function (AnalogInPinMethods) {
        ...
        (AnalogInPinMethods =
            pxsim.AnalogInPinMethods || (pxsim.AnalogInPinMethods = {})),
    )
})(pxsim || (pxsim = {}))
;(function (pxsim) {
    ...
    ;(function (AnalogOutPinMethods) {
        ...
        (AnalogOutPinMethods =
            pxsim.AnalogOutPinMethods || (pxsim.AnalogOutPinMethods = {})),
    )
})(pxsim || (pxsim = {}))
;(function (pxsim) {
    ...
    ;(function (PwmOnlyPinMethods) {
        ...
        (PwmOnlyPinMethods =
            pxsim.PwmOnlyPinMethods || (pxsim.PwmOnlyPinMethods = {})),
    )
})(pxsim || (pxsim = {}))
;(function (pxsim) {
    var pins
    ;(function (pins) {
        ...
        function createI2C(sda, scl) {
            ...
            return (
                b &&
                b.edgeConnectorState &&
                b.edgeConnectorState.createI2C(sda, scl)
            )
        }
        ...
        function createSPI(mosi, miso, sck) {
            ...
            return (
                b &&
                b.edgeConnectorState &&
                b.edgeConnectorState.createSPI(mosi, miso, sck)
            )
        }
...
;(function (pxsim) {
    ...
    ;(function (SerialDeviceMethods) {
        ...
        (SerialDeviceMethods =
            pxsim.SerialDeviceMethods || (pxsim.SerialDeviceMethods = {})),
    )
})(pxsim || (pxsim = {}))
;(function (pxsim) {
    var serial
    ;(function (serial) {
        function internalCreateSerialDevice(tx, rx, id) {
            const b = pxsim.board()
            return b && b.edgeConnectorState
                ? b.edgeConnectorState.createSerialDevice(tx, rx, id)
                : new pxsim.SerialDevice(tx, rx, id)
        }
    ...
    <ellipse style="fill: rgb(216, 216, 216);" cx="21.5" cy="24.181" rx="7" ry="7"/>
        ...
        class NeoPixelCanvas {
            constructor(pin, cols = 1) {
                ...
                this.canvas = el
                this.background = pxsim.svg.child(el, "rect", {
            ...
            updateViewBox(x, y, w, h) {
                ...
                pxsim.svg.hydrate(this.canvas, {
                    viewBox: `${x} ${y} ${w} ${h}`,
                })
                pxsim.svg.hydrate(this.background, {
                    x: x,
                    y: y,
                    width: w,
                    height: h,
        ...
        `
            ...
            updateStripLoc() {
                ...
                pxsim.svg.hydrate(this.part.el, {
                    transform: `translate(${x} ${y})`,
                }) //TODO: update part's l,h, etc.
            }
            updateState() {
    ...
    ;(function (ShaderMethods) {
        function _mergeImage(dst, src, xy) {
            mergeImage(
                dst,
                src,
                pxsim.ImageMethods.XX(xy),
                pxsim.ImageMethods.YY(xy),
            )
        }
            ...
            for (let x = 0; x < src._width; x++) {
                for (let y = 0; y < src._height; y++) {
                    pxsim.ImageMethods.setPixel(
                        ...
                        y0 + y,
                        Math.min(
                            pxsim.ImageMethods.getPixel(dst, x0 + x, y0 + y),
                            pxsim.ImageMethods.getPixel(src, x, y),
                        ),
                    )
                }
            }
            ...
            for (let x = 0; x < src._width; x++) {
                for (let y = 0; y < src._height; y++) {
                    pxsim.ImageMethods.setPixel(
                        ...
                        buf.data[
                            pxsim.ImageMethods.getPixel(dst, x0 + x, y0 + y) +
                                (pxsim.ImageMethods.getPixel(src, x, y) << 4)
                        ],
                    )
                }
            }
