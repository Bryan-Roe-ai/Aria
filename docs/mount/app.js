...
async function loadDashboard() {
    try {
        ...
        let activityHtml =
            '<div class="status-item"><span class="status-label">No recent activity</span></div>'

        if (
            data.quantum.recent_results &&
            data.quantum.recent_results.length > 0
        ) {
            activityHtml = data.quantum.recent_results
                .map(
                    result => `
...
async function loadChatData() {
    try {
        ...
        const providersHtml = Object.entries(data.providers)
            .map(
                ([name, info]) => `
            <div class="status-item">
        ...
        if (data.recent_conversations && data.recent_conversations.length > 0) {
            const historyHtml = data.recent_conversations
                .map(
                    conv => `
                <div class="result-item">
    ...
    try {
        ...
        const autotrainHtml =
            ...
            Object.keys(status.orchestrators.autotrain.jobs).length > 0
                ? Object.entries(status.orchestrators.autotrain.jobs)
                      .map(
                          ([name, job]) => `
                <div class="job-item">
...
async function trainLoRA() {
    ...
    const max_train_samples = parseInt(
        document.getElementById("loraTrainSamples").value,
    )
    const max_eval_samples = parseInt(
