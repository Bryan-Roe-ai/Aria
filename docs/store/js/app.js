...
function initShopPage() {
    ...
    function renderProducts(options) {
        ...
        if (resultsCount) {
            resultsCount.textContent = buildResultSummary(
                filtered.length,
                state,
            )
        }
        ...
        if (grid) {
            if (filtered.length === 0) {
                grid.innerHTML =
                    '<div class="no-results"><i class="fa-solid fa-box-open"></i><p>No offers match your current filters.</p></div>'
            } else {
    ...
    if (form) {
        form.addEventListener("submit", function (e) {
            ...
            var body =
                ...
                message
            var issueUrl =
                "https://github.com/Bryan-Roe/Aria/issues/new?title=" +
