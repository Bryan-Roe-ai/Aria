...
class ModelComparator {
    ...
    async updateComparisonView() {
        ...
        if (this.selectedModels.length === 0) {
            container.innerHTML =
                '<p style="text-align:center;color:#888;padding:40px">Select models to compare</p>'
            return
        }
    ...
    renderComparisonTable(models) {
        const rows = [
            ...
            },
            {
                ...
                format: (_, m) => {
                    if (m.pre_loss && m.post_loss) {
                        const improvement = (
                            ((m.pre_loss - m.post_loss) / m.pre_loss) *
                            100
                        ).toFixed(2)
                        return `${improvement}%`
                    }
            ...
            },
            {
                label: "Duration",
                key: "duration",
                format: v => (v ? `${Math.round(v / 60)}m` : "N/A"),
            },
            ...
            { label: "Batch Size", key: "batch_size" },
            {
                label: "Learning Rate",
                key: "learning_rate",
                format: v => v?.toExponential(2) || "N/A",
            },
            ...
            { label: "Dataset", key: "dataset" },
            {
                ...
                key: "status",
                format: v =>
                    `<span class="badge badge-${v === "completed" ? "success" : "warning"}">${v || "unknown"}</span>`,
            },
        ]
        ...
        html +=
            '<button class="btn btn-primary" onclick="modelComparator.exportComparison()">📥 Export Comparison</button>'
        html +=
            '<button class="btn btn-secondary" onclick="modelComparator.benchmarkSelected()">⚡ Run Benchmark</button>'
        html +=
            '<button class="btn btn-secondary" onclick="modelComparator.clearSelection()">🗑️ Clear</button>'
        html += "</div>"
    ...
    calculateModelScores(model) {
        const lossScore = model.post_loss
            ? Math.min((1 / model.post_loss) * 10, 100)
            : 0
        const speedScore = model.duration
            ? Math.min((3600 / model.duration) * 100, 100)
            : 0
        const improvementScore =
            model.pre_loss && model.post_loss
                ? Math.min(
                      ((model.pre_loss - model.post_loss) / model.pre_loss) *
                          100,
                      100,
                  )
                : 0
        const efficiencyScore =
            model.batch_size && model.duration
    ...
    async exportComparison() {
        ...
        if (format === "json") {
            ...
            const rows = models.map(m => [
                ...
                m.pre_loss && m.post_loss
                    ? (((m.pre_loss - m.post_loss) / m.pre_loss) * 100).toFixed(
                          2,
                      )
                    : "",
    ...
    async benchmarkSelected() {
        if (this.selectedModels.length === 0) {
            if (typeof showToast === "function")
                showToast("Please select models to benchmark", "warn", 4000)
            return
        }
        ...
        if (container) {
            container.innerHTML =
                '<div class="alert alert-info">Running benchmark... This may take a few minutes.</div>'
        }
    ...
    displayBenchmarkResults(results) {
        ...
        let html =
            '<div class="card"><div class="card-header"><h3>⚡ Benchmark Results</h3></div><table>'
        html +=
            "<thead><tr><th>Model</th><th>Inference Time (ms)</th><th>Memory (MB)</th><th>Throughput (tok/s)</th><th>Score</th></tr></thead>"
        html += "<tbody>"
    ...
    calculateBenchmarkScore(result) {
        const speedScore = result.inference_time
            ? Math.min(100, (100 / result.inference_time) * 100)
            : 0
        const memoryScore = result.memory_mb
            ? Math.min(100, (2000 / result.memory_mb) * 100)
            : 0
        const throughputScore = result.throughput
            ? Math.min(100, result.throughput)
            : 0
    ...
    async generateLeaderboard() {
        try {
            ...
            html +=
                "<thead><tr><th>Rank</th><th>Model</th><th>Loss</th><th>Duration</th><th>Score</th></tr></thead>"
            html += "<tbody>"
