...
class TrainingAnomalyDetector {
    constructor(options = {}) {
        this.lossHistory = []
        this.config = {
            spikeThreshold: options.spikeThreshold || 0.2, // 20% increase
    ...
    detectAnomalies() {
        ...
        if (
            previous &&
            current.loss > previous.loss * (1 + this.config.spikeThreshold)
        ) {
            const increase = (
                ((current.loss - previous.loss) / previous.loss) *
                100
            ).toFixed(1)
            this.reportAnomaly("spike", {
                ...
                recommendation:
                    "Check for data quality issues or reduce learning rate",
            })
        }
        ...
        if (current.loss > this.config.divergenceThreshold) {
            this.reportAnomaly("divergence", {
                ...
                jobName: current.jobName,
                recommendation:
                    "Training likely unstable. Reduce learning rate significantly or restart with different hyperparameters",
            })
        }
        ...
        if (recent.length >= this.config.stagnationEpochs) {
            ...
            if (variation < 0.001) {
                this.reportAnomaly("stagnation", {
                    ...
                    jobName: current.jobName,
                    recommendation:
                        "Consider early stopping or increasing learning rate",
                })
            }
        }
    }
    ...
    reportAnomaly(id, anomaly) {
        ...
        const recentDuplicate = this.anomalies.find(
            a =>
                a.type === anomaly.type &&
    ...
    getStatistics() {
        return {
            ...
            divergences: this.anomalies.filter(
                a => a.type === "Training Divergence",
            ).length,
            stagnations: this.anomalies.filter(
                a => a.type === "Training Stagnation",
            ).length,
