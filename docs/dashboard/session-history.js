...
class TrainingSessionHistory {
    ...
    async loadSessions() {
        if (this.config.storage === "localStorage") {
            ...
            console.warn(
                "IndexedDB storage not yet implemented, falling back to localStorage",
            )
    ...
    filterSessions(filters = {}) {
        ...
        if (filters.model) {
            filtered = filtered.filter(
                s => s.config.model && s.config.model.includes(filters.model),
            )
        }
        ...
        if (filters.dataset) {
            filtered = filtered.filter(
                s =>
                    s.config.dataset &&
                    s.config.dataset.includes(filters.dataset),
            )
        }
        ...
        if (filters.minLoss !== undefined) {
            filtered = filtered.filter(
                s =>
                    s.metrics.finalLoss !== null &&
                    s.metrics.finalLoss >= filters.minLoss,
            )
        }
        ...
        if (filters.maxLoss !== undefined) {
            filtered = filtered.filter(
                s =>
                    s.metrics.finalLoss !== null &&
                    s.metrics.finalLoss <= filters.maxLoss,
            )
        }
    ...
    compareSessions(sessionIds) {
        ...
        return {
            ...
            comparison: {
                bestLoss: Math.min(
                    ...sessions.map(s => s.metrics.finalLoss || Infinity),
                ),
                worstLoss: Math.max(
                    ...sessions.map(s => s.metrics.finalLoss || 0),
                ),
                avgLoss:
                    sessions.reduce(
                        (sum, s) => sum + (s.metrics.finalLoss || 0),
                        0,
                    ) / sessions.length,
            ...
            bestSession:
                completed.length > 0
                    ? completed.reduce((best, s) =>
                          (s.metrics.finalLoss || Infinity) <
                          (best.metrics.finalLoss || Infinity)
                              ? s
                              : best,
                      )
                    : null,
