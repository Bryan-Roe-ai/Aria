;(function () {
    ...
    function detectSiteKind() {
        ...
        if (
            ...
            target.includes("chat/index") ||
            target.endsWith("/chat")
        ) {
            return "chat"
        }

        if (
            target.includes("/aria/") ||
            target.includes("auto-execute") ||
            target.includes("aria/index")
        ) {
            return "aria"
        }
        ...
        if (
            ...
            target.includes("dashboard/index") ||
            target.includes("dashboard/hub")
        ) {
            return "dashboard"
        }
        ...
        if (
            ...
            target.endsWith("/docs") ||
            target.includes("readme_pages")
        ) {
            return "docs"
        }
        ...
        if (
            ...
            target.includes("/monetization/") ||
            target.includes("llm-maker") ||
            target.includes("/mount/")
        ) {
            return "dashboard"
        }
    ...
    function shouldOpenDebugFromQuery() {
        ...
        var raw =
            params.get("passpanel") ||
            params.get("debugpasses") ||
            params.get("passdebug")
        if (!raw) {
            return null
        }
    ...
    function shouldAutoplayFromQuery() {
        ...
        var raw =
            params.get("passauto") ||
            params.get("passautoplay") ||
            params.get("autosweep")
        if (!raw) {
            return null
        }
    ...
    function getConfiguredPassCount(kind) {
        ...
        var queryRaw =
            params.get("passes") ||
            params.get("pass") ||
            (stateFromQuery && stateFromQuery.passCount)
        var savedRaw = null

        try {
            if (window.localStorage) {
                savedRaw =
                    localStorage.getItem(getPassStorageKey(kind)) ||
                    localStorage.getItem(PASS_STORAGE_KEY)
            }
        } catch (error) {
            savedRaw = null
        }
    ...
    function getConfiguredAutoplayInterval(kind) {
        ...
        var stateFromQuery = getPassStateFromQuery()
        var queryRaw =
            params.get("passsweepms") ||
            params.get("passautoplayms") ||
            params.get("passspeed") ||
            (stateFromQuery && stateFromQuery.sweepMs)
        var savedRaw = null

        try {
            if (window.localStorage) {
                savedRaw =
                    localStorage.getItem(getPassSpeedStorageKey(kind)) ||
                    localStorage.getItem(PASS_AUTOPLAY_STORAGE_KEY)
            }
        } catch (error) {
            savedRaw = null
        }
        ...
        return Math.round(
            clamp(parsed, PASS_AUTOPLAY_MIN_MS, PASS_AUTOPLAY_MAX_MS),
        )
    }
    ...
    function normalizePassState(rawState) {
        ...
        var passCount = toValidPass(
            rawState.passCount !== undefined
                ? rawState.passCount
                : rawState.pass !== undefined
                  ? rawState.pass
                  : rawState.p,
        )
        var sweepMs = toValidSpeed(
            rawState.sweepMs !== undefined
                ? rawState.sweepMs
                : rawState.speed !== undefined
                  ? rawState.speed
                  : rawState.ms,
        )
        var snapshotA = toValidPass(
            rawState.snapshotA !== undefined ? rawState.snapshotA : rawState.a,
        )
        var snapshotB = toValidPass(
            rawState.snapshotB !== undefined ? rawState.snapshotB : rawState.b,
        )
    ...
    function getPassStateFromQuery() {
        return getPassStateFromSearchParams(
    ...
    function setPassVariables(kind, passIndex, totalPasses) {
        ...
        root.style.setProperty(
            "--pass-radius-boost",
            radiusBoost.toFixed(2) + "px",
        )
        root.style.setProperty(
            "--pass-focus-width",
            focusWidth.toFixed(2) + "px",
        )
        root.style.setProperty(
            "--pass-letter-shift",
            letterShift.toFixed(4) + "em",
        )
    ...
    function emitPassChange(kind, passCount, source) {
        ...
        document.dispatchEvent(
            new CustomEvent("site-upgrade:pass-change", { detail: detail }),
        )
    ...
    function runPolishPasses(kind, requestedPassCount, source) {
        var passCount = Math.round(
            clamp(Number(requestedPassCount), PASS_MIN, PASS_MAX),
        )
    ...
    function createPassDebugPanel(kind, initialPassCount) {
        ...
        var stepButtons = panel.querySelectorAll(
            ".site-pass-debug__btn[data-step]",
        )
        var presetButtons = panel.querySelectorAll(
            ".site-pass-debug__btn[data-preset]",
        )
        var resetButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="reset"]',
        )
        var closeButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="close"]',
        )
        var defaultButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="default"]',
        )
        var autoplayButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="autoplay"]',
        )
        var undoButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="undo"]',
        )
        var redoButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="redo"]',
        )
        var saveAButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="save-a"]',
        )
        var saveBButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="save-b"]',
        )
        var applyAButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="apply-a"]',
        )
        var applyBButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="apply-b"]',
        )
        var toggleAbButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="toggle-ab"]',
        )
        var copyUrlButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="copy-url"]',
        )
        var applyUrlButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="apply-url"]',
        )
        var pushUrlButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="push-url"]',
        )
        var clearHistoryButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="clear-history"]',
        )
        var exportJsonButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="export-json"]',
        )
        var toggleBlurButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="toggle-blur"]',
        )
        var resetAppearanceButton = panel.querySelector(
            '.site-pass-debug__btn[data-action="reset-appearance"]',
        )
        ...
        var resizeHandle = panel.querySelector(
        ...
        var shouldAutoStart =
            queryState && queryState.auto !== null ? queryState.auto : queryAuto
        ...
        var autoplayIntervalMs =
            queryState && Number.isFinite(queryState.sweepMs)
                ? queryState.sweepMs
        ...
        function persistSnapshots() {
            trySaveValue(
                getPassSnapshotStorageKey(kind),
                JSON.stringify({
                    a: snapshots.a,
                    b: snapshots.b,
                }),
        ...
        function syncSweepLabel() {
            ...
            sweepValue.textContent =
                (isAutoplay ? "running" : "idle") +
                " @ " +
                autoplayIntervalMs +
                "ms"
        }
        ...
        function shouldTrackUndo(source, nextPass) {
            ...
            if (
                normalized === "autoplay" ||
                normalized.indexOf("undo") !== -1 ||
                normalized.indexOf("redo") !== -1 ||
                normalized === "state-init" ||
                normalized === "event"
            ) {
                return false
            }
        ...
        function renderHistory() {
            ...
            passHistory.forEach(function (entry) {
                ...
                item.className =
                    "site-pass-debug__timeline-item site-pass-debug__timeline-item--restore"
                item.tabIndex = 0
                item.setAttribute("role", "button")
                item.title = "Restore pass " + entry.pass
                item.setAttribute(
                    "aria-label",
                    "Restore " + entry.pass + " passes (" + entry.source + ")",
                )
                ...
                var deltaText =
                    entry.delta === null
                        ...
                        : " (" +
                          ...
                          ")"
                detail.textContent =
                    entry.source + " -> " + entry.pass + deltaText
        ...
        function syncSnapshotUI(passCount) {
            ...
            saveAButton.classList.toggle(
                "is-active",
                hasA && snapshots.a === passCount,
            )
            saveBButton.classList.toggle(
                "is-active",
                hasB && snapshots.b === passCount,
            )
        ...
        function applySnapshotToggle(source) {
            ...
            if (hasA && hasB) {
                var target =
                    currentPasses === snapshots.a ? snapshots.b : snapshots.a
                applyPassCount(target, true, source || "snapshot-toggle")
                return
            }

            applyPassCount(
                hasA ? snapshots.a : snapshots.b,
                true,
                source || "snapshot-toggle",
            )
        }
        ...
        function setAutoplayInterval(rawInterval, persistSelection) {
            ...
            if (persistSelection) {
                trySaveValue(
                    getPassSpeedStorageKey(kind),
                    String(autoplayIntervalMs),
                )
                trySaveValue(
                    PASS_AUTOPLAY_STORAGE_KEY,
                    String(autoplayIntervalMs),
                )
            }
        ...
        function syncFromPassEvent(event) {
            ...
            currentPasses = Math.round(
                clamp(detailPassCount, PASS_MIN, PASS_MAX),
            )
            syncInputs(currentPasses)
        ...
        function applyPassCount(rawPassCount, persistSelection, source) {
            var normalized = Math.round(
                clamp(Number(rawPassCount), PASS_MIN, PASS_MAX),
            )
        ...
        if (
            queryState &&
            (queryState.snapshotA !== null || queryState.snapshotB !== null)
        ) {
            persistSnapshots()
        }
        ...
        ;(function () {
            ...
            function resetPosition() {
                ...
                panel.style.bottom = ""
                try {
                    localStorage.removeItem(getPassPositionStorageKey(kind))
                } catch (e) {
                    /* */
                }
            }
            ...
            document.addEventListener("mouseup", function (evt) {
                ...
                trySaveValue(
                    getPassPositionStorageKey(kind),
                    JSON.stringify({ right: finalRight, bottom: finalBottom }),
                )
                dragState = null
            })

            dragHead.addEventListener(
                "touchstart",
                function (evt) {
        ...
        function showShortcutOverlay() {
            ...
            var closeBtn = shortcutOverlay.querySelector(
            ...
            function dismissOverlay(evt) {
                ...
                var box = shortcutOverlay.querySelector(
        ...
        if (savedSections) {
            sectionEls.forEach(function (el) {
                ...
                if (
                    name &&
                    Object.prototype.hasOwnProperty.call(savedSections, name)
                ) {
        ...
        function animateDetails(el, opening) {
            ...
            if (el._animating) {
                ...
                el._animating = null
                if (el._innerEl) {
                    el._innerEl.style.height = ""
                    el._innerEl.style.overflow = ""
                }
            }
            ...
            if (!wrapper) {
                ...
                children.forEach(function (node) {
                    if (node.nodeType === 1 && node.tagName === "SUMMARY") {
                        return
                    }
            ...
            el.addEventListener("click", function (evt) {
                if (evt.target && evt.target.tagName === "SUMMARY") {
                    ...
                    sectionEls.forEach(function (s) {
                        ...
                        if (sName) {
                            state[sName] = willOpen ? true : false
                        }
                    })
                    ...
                    sectionEls.forEach(function (s) {
                        ...
                        if (n) {
                            fullState[n] = n === secName ? willOpen : s.open
                        }
                    })
                    trySaveValue(
                        getPassSectionsStorageKey(kind),
        ...
        ;(function () {
            ...
            function applyWidth(w) {
                var clamped = Math.min(
                    ...
                    Math.min(560, window.innerWidth - 28),
                )
            ...
            function resetWidth() {
                ...
                try {
                    localStorage.removeItem(getPassWidthStorageKey(kind))
                } catch (e) {
                    /* */
                }
            }
                ...
                if (navigator.clipboard && navigator.clipboard.writeText) {
                    navigator.clipboard
                        ...
                        .then(function () {
                            ...
                            setTimeout(function () {
                                exportJsonButton.textContent = orig
                            }, 1200)
                        })
                        .catch(function () {
        ...
        ;(function () {
            ...
            var currentOpacity =
                saved && Number.isFinite(saved.opacity) ? saved.opacity : 100
            var currentBlur =
                saved && typeof saved.blur === "boolean" ? saved.blur : true
            ...
            function applyAppearance(opacity, blur) {
                ...
                innerPanel.style.webkitBackdropFilter = blur
                    ? "blur(10px)"
                    : "none"
            }
            ...
            function persistAppearance() {
                trySaveValue(
                    getPassPanelStyleStorageKey(kind),
                    JSON.stringify({
            ...
            if (opacityInput) {
                opacityInput.addEventListener("change", function () {
                    var v = Math.min(
                        100,
                        Math.max(20, Number(opacityInput.value) || 100),
                    )
                    currentOpacity = v
