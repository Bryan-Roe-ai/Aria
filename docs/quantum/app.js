...
async function updateTrainingStatus() {
    ...
    try {
        ...
        document.getElementById("best-val-acc").textContent =
            (status.best_val_acc * 100).toFixed(2) + "%"
...
async function exportMetrics() {
    ...
    try {
        const response = await fetch(
            `${API_BASE}/api/export/metrics/${currentSessionId}`,
        )
...
async function evaluateNow() {
    ...
    try {
        const res = await fetch(
            `${API_BASE}/api/train/evaluate/${currentSessionId}`,
        )
