...
def list_values(limit: int = 100) -> list[dict]:  # noqa: ANN001
    ...
    if not engine and not _SQLALCHEMY_AVAILABLE:
        try:
            conn = _get_sqlite_conn()
            cur = conn.execute("SELECT key_name, value_data, updated_at FROM QAI_KeyValue ORDER BY updated_at DESC")
            out = []
            for row in cur.fetchall()[:limit]:
                out.append({"key_name": row[0], "value_data": row[1], "updated_at": row[2]})
            return out
        except Exception as e:  # noqa: BLE001
    ...
    try:
        with engine.connect() as conn:
            res = conn.execute(text("SELECT key_name, value_data, updated_at FROM QAI_KeyValue ORDER BY updated_at DESC"))
            items = []
            for row in res.fetchall()[:limit]:
                items.append({"key_name": row[0], "value_data": row[1], "updated_at": row[2]})
            return items
    except Exception as e:  # noqa: BLE001
