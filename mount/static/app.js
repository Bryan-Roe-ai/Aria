...
async function loadDashboard() {
    try {
        ...
        document.getElementById("systemStatus").innerHTML = emptyRow(
...
async function loadQuantumData() {
    try {
        ...
        document.getElementById("quantumAutorunJobs").innerHTML = emptyRow(
...
async function loadTrainingData() {
    try {
        ...
        document.getElementById("loraAdapterStatus").innerHTML =
            adapter.available
                ? `
                <div class="status-item">
        ...
        fillSelect(
            "autotrainJob",
            (jobs.jobs || []).map(j => ({ value: j, label: j })),
            "Select job…",
        )

        const autotrainJobs = status.orchestrators?.autotrain?.jobs || {}
        document.getElementById("autotrainStatus").innerHTML = Object.keys(
            autotrainJobs,
        ).length
            ? Object.entries(autotrainJobs)
                  .map(
                      ([name, job]) => `
                <div class="job-item">
...
function initializeForms() {
    document
        .getElementById("quantumTrainForm")
        .addEventListener("submit", async e => {
            e.preventDefault()
            await trainQuantumClassifier(
                e.submitter || e.target.querySelector('[type="submit"]'),
            )
        })
    document
        .getElementById("loraTrainForm")
        .addEventListener("submit", async e => {
            e.preventDefault()
            await trainLoRA(
                e.submitter || e.target.querySelector('[type="submit"]'),
...
async function trainQuantumClassifier(btn) {
    ...
    if (!dataset) {
        toast("Please select a dataset", "warning")
        return
    }
...
async function trainLoRA(btn) {
    ...
    const payload = {
        dataset,
        max_train_samples: parseInt(
            document.getElementById("loraTrainSamples").value,
            10,
        ),
        max_eval_samples: parseInt(
            document.getElementById("loraEvalSamples").value,
            10,
        ),
...
async function runAutoTrain(dryRun, btn) {
    ...
    if (!job) {
        toast("Please select a job", "warning")
        return
    }
    ...
    try {
        const result = await fetchJSON(
            "/training/autotrain",
            jsonPost({ job_name: job, dry_run: dryRun }),
        )
        if (result.success) {
...
function refreshStatus(btn) {
    ...
    setLoading(btn, true)
    Promise.all([checkServiceStatus(), loadDashboard()]).finally(() =>
        setLoading(btn, false),
    )
}
