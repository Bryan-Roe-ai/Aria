...
class OllamaCompatibleHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/api/tags':
            ...
            response = {
                "models": [
                    {"name": "mistral:latest", "modified_at": datetime.now().isoformat()},
                    {"name": "neural-chat:latest", "modified_at": datetime.now().isoformat()},
                    {"name": "codellama:latest", "modified_at": datetime.now().isoformat()}
                ]
            }
