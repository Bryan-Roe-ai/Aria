    ...
    try:
        ...
        try:
            ...
            result["mem_percent"] = round(
                used / total * 100, 1) if total else 0.0
        except Exception:
            pass
        return result
...
def _collect_disk() -> List[Dict[str, Any]]:
    try:
        ...
        import subprocess
        try:
            out = subprocess.check_output(["df", "-BG", "--output=target,source,fstype,size,used,avail,pcent"],
                                          text=True, timeout=5)
            rows = []
...
def _collect_gpu() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        out = subprocess.check_output(
            ["nvidia-smi", f"--query-gpu={qfields}",
        ...
        for line in out.strip().splitlines():
            ...
            if len(parts) < 7:
                continue
            mem_used = float(parts[3]) if parts[3] not in (
                "[N/A]", "N/A") else 0
            mem_total = float(parts[4]) if parts[4] not in (
                "[N/A]", "N/A") else 1
    ...
    try:
        ...
        for i in range(torch.cuda.device_count()):
            ...
            pct = round(mem_used_bytes / mem_total_bytes *
                        100, 1) if mem_total_bytes else 0
            gpus.append({
                "index":           i,
                "name":            props.name,
                "gpu_util":        None,
...
def print_snapshot(snap: Dict[str, Any]):
    ...
    print(
        f"  MEM  {_badge(lvl)}  {_bar(mem_pct)}   {mem_used:.1f}/{mem_total:.1f} GB")
    ...
    if gpus:
        ...
        for g in gpus:
            ...
            if util_pct is not None:
                print(
                    f"      util {_badge(lvl_util)}  {_bar(util_pct)}{temp_str}")
            print(
...
def main():
    ...
    parser.add_argument("--snapshot",   action="store_true",
                        help="One-shot snapshot (default)")
    parser.add_argument("--watch",      action="store_true",
                        help="Continuous monitoring")
    parser.add_argument("--interval",   type=int, default=5,
                        help="Refresh interval in seconds")
    parser.add_argument("--export",     nargs="?", const="data_out/resource_snapshot.json",
                        metavar="FILE", help="Export snapshot to JSON")
