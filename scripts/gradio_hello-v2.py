...
# ruff: noqa: E501,E203
# pycodestyle: ignore=E501,E203
# pyright: reportMissingImports=false
# pylint: disable=import-error,line-too-long,missing-function-docstring,redefined-outer-name,broad-exception-caught,too-many-return-statements,too-many-arguments,too-many-positional-arguments,too-many-locals,too-many-branches,too-many-statements,invalid-name,unnecessary-lambda,unused-argument,too-many-lines
...
.gradio-chatbot > div::-webkit-scrollbar {
    width: 5px;
}
...
def load_latest_conversation() -> tuple[
    list[dict[str, str]],
    list[dict[str, Any]],
]:
...
def provider_status_snapshot(provider_choice: str) -> tuple[str, str]:
    ...
    if selected == "lmstudio":
        ...
        return (
            "lmstudio",
            "LM Studio selected but not configured in this environment.",
        )
    if selected == "azure":
        if all(
            os.getenv(name)
        ...
        return (
            "azure",
            (
                "Azure OpenAI selected but required environment "
...
def reset_chat_session(
    ...
    provider_name, provider_status = provider_status_snapshot(
        provider_choice
    )
    return [], [], provider_name, provider_status
...
def _append_turn(
    ...
    return (
        next_chat_history[-int(max_history * 2):],
        next_hist_state[-int(max_history):],
    )


def _autosave_history(
    hist_state: list[dict[str, Any]],
    session_label: str,
    autosave: bool,
) -> None:
    if autosave:
        try:
            save_conversation_json(hist_state, session_label or "session")
        except Exception:
            pass


def _save_json_if_present(
    conversation_history: list[dict[str, Any]],
    session_label: str,
):
    if not conversation_history:
        return None
    return save_conversation_json(
        conversation_history,
        session_label or "session",
    )


def _save_markdown_if_present(
    conversation_history: list[dict[str, Any]],
    session_label: str,
):
    if not conversation_history:
        return None
    return save_conversation_markdown(
        conversation_history,
        session_label or "session",
    )


def _save_txt_if_present(
    conversation_history: list[dict[str, Any]],
    session_label: str,
):
    if not conversation_history:
        return None
    return save_conversation_txt(
        conversation_history,
        session_label or "session",
    )


def _load_latest_pair():
    return load_latest_conversation()


def _quick_prompt_priorities() -> str:
    return "Summarize today's priorities in 5 bullets."


def _quick_prompt_update() -> str:
    return "Help me draft a short project update."


def _quick_prompt_ideas() -> str:
    return "Give me three creative app ideas."


def _display_all(hist: list[dict[str, Any]]) -> list[dict[str, str]]:
    return hist_state_to_display(hist)
...
def respond(
    ...
    if use_model:
        ...
        chat_history, hist_state = _append_turn(
            chat_history,
            hist_state,
            user_message,
            reply,
        ...
        _autosave_history(hist_state, session_name, autosave)
        yield (
            chat_history,
            "",
            hist_state,
            "simulation",
            "Replied with simulation mode.",
        )
        return
...
with gr.Blocks() as demo:
    with gr.Column(elem_id="appCard"):
        gr.HTML(
            ...
            <div class="hero-banner">
                <div style="display:flex;align-items:center;gap:14px;">
                    <span
                        style="font-size:2.4rem;line-height:1;filter:drop-shadow(0 2px 6px rgba(20,83,45,0.16));"
                    >🌿</span>
                    <div>
                        <div class="hero-title">Aria Chat</div>
                        <p class="hero-subtitle" style="margin:4px 0 0;">
                            Conversation-first AI assistant &mdash; provider,
                            export, and history controls stay out of the way
                            until you need them.
                        </p>
                    </div>
                </div>
        ...
        with gr.Row(equal_height=False):
            ...
            with gr.Column(
                ...
                gr.Markdown(
                    ...
                    "Simple mode hides advanced settings to keep the UI easy."
                    "</div>"
                )
                ...
                gr.Markdown(
                    "<div class='simple-note'>"
                    f"{provider_diagnostics_summary()}"
                    "</div>"
                )
                ...
                with gr.Column(visible=False) as advanced_controls:
                    ...
                    with gr.Accordion("History and Sessions", open=False):
                        ...
                        session_name = gr.Textbox(
                            label="Session name",
                            placeholder="session-2026-05-29",
                        )
                        with gr.Row():
                        ...
                        saved_sessions = gr.Dropdown(
                            choices=list_json_sessions(),
                            label="Saved sessions (.json)",
                        )
                        with gr.Row():
                    ...
                    with gr.Accordion("Search and Audio", open=False):
                        search_input = gr.Textbox(
                            ...
                            placeholder="Search user or assistant text",
                        )
                        with gr.Row():
                        ...
                        tts_autoplay = gr.Checkbox(
                            label="Autoplay assistant audio",
                            value=False,
                        )
                        speak_btn = gr.Button(
        ...
        greet_btn.click(
            make_greeting,
            inputs=[name, style, excitement, language],
            outputs=output,
        )
        ...
        qp1.click(
            _quick_prompt_priorities,
            outputs=[user_input],
        )
        qp2.click(
            _quick_prompt_update,
            outputs=[user_input],
        )
        qp3.click(
            _quick_prompt_ideas,
        ...
        export_md_btn.click(
            _save_markdown_if_present,
            inputs=[hist_state, session_name],
            outputs=[export_file],
        )
        export_txt_btn.click(
            _save_txt_if_present,
            inputs=[hist_state, session_name],
            outputs=[export_file],
        )
        ...
        def delete_session(session_file: str):
            if session_file:
                try:
                    (CONV_DIR / session_file).unlink(missing_ok=True)
                except Exception:
                    pass
            files = list_json_sessions()
            return gr.Dropdown(choices=files, value=files[0] if files else None)
        ...
        delete_session_btn.click(
            delete_session,
            inputs=[saved_sessions],
            outputs=[saved_sessions],
        )

        def search_chat(query: str, conversation_history: list[dict[str, Any]]):
            ...
            filtered = [
                ...
                if q in e.get("user", "").lower()
                or q in e.get("assistant", "").lower()
            ]
            return hist_state_to_display(filtered)

        search_btn.click(
            search_chat,
            inputs=[search_input, hist_state],
            outputs=[chatbot],
        )
        revert_btn.click(_display_all, inputs=[hist_state], outputs=[chatbot])
        ...
        def speak_last(
            ...
            if not conversation_history:
                return None
            return generate_tts_for_text(
                conversation_history[-1].get("assistant", "")
            )

        speak_btn.click(
            speak_last,
            inputs=[hist_state, tts_autoplay],
            outputs=[tts_audio],
        )
