...
def test_imports():
    ...
    try:
        ...
        logger.info("✅ Quantum modules (quantum_classifier, hybrid_qnn) available")
        tests_passed += 1
    except ImportError:
        logger.warning("⚠️ Quantum modules not available (expected in some environments)")
...
def test_configuration():
    ...
    try:
        ...
        required_keys = ["quantum_settings", "passive_training", "llm_training"]
    ...
    try:
        ...
        if "quantum_llm" not in autonomous_config:
            logger.error("❌ autonomous_training.yaml missing quantum_llm config")
            tests_failed += 1
        elif not autonomous_config.get("quantum_llm", {}).get("enabled"):
            logger.warning("⚠️ quantum_llm.enabled is False in autonomous config")
        else:
            logger.info("✅ autonomous_training.yaml has quantum_llm integration")
            tests_passed += 1
    except Exception as e:
...
def test_optimizer_and_encoder():
    ...
    try:
        import torch
        from quantum_llm_trainer import QuantumAttentionOptimizer, QuantumFeatureEncoder
...
def test_orchestrator_config():
    ...
    try:
        ...
        required_keys = ["enabled", "passive_mode", "backend", "training_interval_minutes"]
        missing = [k for k in required_keys if k not in quantum_config]
        ...
        if not quantum_script.exists():
            ...
            logger.info("✅ quantum_llm_trainer.py script exists and is accessible")
            tests_passed += 1
...
def test_repo_automation():
    ...
    try:
        ...
        if "autonomous_training_orchestrator.py" in content:
            logger.info("✅ repo_automation.py calls autonomous_training_orchestrator")
            tests_passed += 1
        else:
            logger.warning("⚠️ repo_automation.py may not call orchestrator properly")
...
def generate_integration_report():
    ...
    for name, result in results.items():
        ...
        logger.info(f"{status} {name}: {result['passed']} passed, {result['failed']} failed")
    
    logger.info(f"\nTotal: {all_passed} passed, {all_failed} failed")
    
    if all_failed == 0:
        logger.info("\n🎉 ALL INTEGRATION TESTS PASSED!")
        logger.info("\nQuantum-Enhanced LLM Training is fully integrated and ready for use:")
        logger.info("  • standalone: python scripts/quantum_llm_trainer.py --dataset ...")
        logger.info("  • passive: python scripts/quantum_llm_trainer.py --passive --config ...")
        logger.info("  • autonomous: python scripts/autonomous_training_orchestrator.py")
