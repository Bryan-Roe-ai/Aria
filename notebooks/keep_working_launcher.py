...
def export_systemd(path, python_exec=None, workspace_dir=None, task='Keep Working', work=1500, short=300, long=900, cycles_per_long=4, notify=False, sound=False, repeat=False):
    p = Path(path).expanduser()
    p.parent.mkdir(parents=True, exist_ok=True)
    ...
    args = [python_exec, launcher, '--task', task, '--work', str(work), '--short', str(short), '--long', str(long), '--cycles-per-long', str(cycles_per_long)]
    if notify:
        args.append('--notify')
    if sound:
        args.append('--sound')
...
def run_session(seconds, task_name, kind='work', notify=False, sound=False):
    ...
    rec = SessionRecord(task_name=task_name, start_ts=start.isoformat(), end_ts=end.isoformat(), duration_s=seconds, kind=kind)
...
def main():
    ...
    p.add_argument('--cycles-per-long', type=int, dest='cycles_per_long', help='Number of work cycles before a long break')
    ...
    p.add_argument('--notify', action='store_true', default=None, help='Show desktop notification when a session finishes')
    p.add_argument('--sound', action='store_true', default=None, help='Play a short sound (bell) when a session finishes')
    p.add_argument('--status-file', type=str, dest='status_file', default=None, help='Write a JSON status file with last_session and running state')
    p.add_argument('--export-systemd', type=str, default=None, help='Export a systemd user service file to the given path and exit')
    p.add_argument('--install-systemd', action='store_true', help='Write service to ~/.config/systemd/user/keep-working.service and print enable instructions')
    p.add_argument('--no-config', action='store_true', help='Ignore notebooks/keep_working_config.json and use built-in defaults')
    ...
    if not any(a == '--no-config' for a in sys.argv[1:]):
        ...
        p.set_defaults(task='Unnamed', work=25*60, short=5*60, long=15*60,
                       cycles_per_long=4, notify=False, sound=False, repeat=False,
                       status_file=str(Path('notebooks') / 'keep_working_status.json'))
    ...
    if args.export_systemd:
        svc_path = export_systemd(args.export_systemd, python_exec=sys.executable, workspace_dir=str(Path.cwd()), task=args.task, work=args.work, short=args.short, long=args.long, cycles_per_long=args.cycles_per_long, notify=args.notify, sound=args.sound, repeat=args.repeat)
        print('Wrote systemd service to', svc_path)
        print('To enable: copy to ~/.config/systemd/user/ and run `systemctl --user daemon-reload && systemctl --user enable --now <name>`')
        return
    if args.install_systemd:
        user_path = Path('~/.config/systemd/user/keep-working.service').expanduser()
        svc_path = export_systemd(str(user_path), python_exec=sys.executable, workspace_dir=str(Path.cwd()), task=args.task, work=args.work, short=args.short, long=args.long, cycles_per_long=args.cycles_per_long, notify=args.notify, sound=args.sound, repeat=args.repeat)
