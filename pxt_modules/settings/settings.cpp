...
#define NRF_BOOTLOADER_START *(uint32_t *)0x10001014
        fs(flash,
           128 * 1024 < NRF_BOOTLOADER_START &&
                   NRF_BOOTLOADER_START < (uint32_t)flash.totalSize()
...
// it may be already occupied by the user program, in which case
// largeStoreStart() will return 0
...
CODAL_FLASH *largeStoreFlash() { return &getWStorage()->flash; }
...
static bool isSystem(const char *fn) { return fn[0] == '#'; }
