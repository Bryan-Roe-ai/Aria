...
void FS::writeBytes(void *dst, const void *src, uint32_t size) {
  LOGVV("write %x%s %d %x:%x:%x:%x", REAL_OFF(dst),
        OFF(dst) == REAL_OFF(dst) ? "" : "*", size, ((const uint8_t *)src)[0],
        ((const uint8_t *)src)[1], ((const uint8_t *)src)[2],
        ((const uint8_t *)src)[3]);
...
bool FS::tryMount() {
  ...
  LOG("mounted, end=%x meta=%x free=%x", OFF(endPtr), OFF(metaPtr),
      OFF(freeDataPtr));
...
int FS::remove(const char *keyName) { return write(keyName, NULL, M1); }
...
uint16_t FS::writeData(const void *data, uint32_t len) {
  LOGVV("writeData: @%x %x:%x sz=%d", REAL_OFF(freeDataPtr),
        ((const uint8_t *)data)[0], ((const uint8_t *)data)[1], len);
