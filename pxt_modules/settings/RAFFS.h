...
struct MetaEntry {
  ...
  uint16_t
      _datasize; // size in bytes; highest bit is set if this isn't first block
...
public:
  // Minimum time in ms that has to pass between two GCs; we shall panic 920 if
  // GCs happen more often (avareged over 3x this time). This is usually set to
  // around 10s (10000), so that if user writes a program that writes to flash
  // in a loop, it doesn't wear out flash completely.
  ...
  DirEntry *dirRead(); // data is only valid until next call to to any of File
                       // or FS function
