namespace control {
    ...
    export function onEvent(
        ...
        if (!ctx) control.internalOnEvent(src, value, handler, flags)
        else ctx.registerHandler(src, value, handler, flags)
    }
    ...
    export class EventContext {
        ...
        private runCallbacks() {
            ...
            if (this.timeInSample > 1000 || this.framesInSample > 30) {
                ...
                if (control.profilingEnabled()) {
                    control.dmesg(
                        `${(fps * 100) | 0}/100 fps - ${this.framesInSample} frames (${this.timeInSample}ms/${realTimeInSample}ms)`,
                    )
                    control.gc()
        ...
        register() {
            for (const h of this.handlers) h.register()
            this.registerFrameCallbacks()
        }

        unregister() {
            for (const h of this.handlers) h.unregister()
            this.frameWorker++
        }
    ...
    export function eventContext(): EventContext {
        return eventContexts
            ? eventContexts[eventContexts.length - 1]
            : undefined
    }
    ...
    export function pushEventContext(): EventContext {
        if (!eventContexts) eventContexts = []
