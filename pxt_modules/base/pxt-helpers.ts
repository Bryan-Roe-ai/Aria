...
function parseInt(text: string, radix?: number): number {
    ...
    if (!text || (radix != null && (radix < 2 || radix > 36))) return NaN
...
namespace helpers {
    ...
    */
    ...
    export function arraySome<T>(
        ...
        let len = arr.length // caching this seems to match V8
        for (let i = 0; i < len; ++i) if (callbackfn(arr[i], i)) return true
        return false
    }
    ...
    export function arrayEvery<T>(
        ...
        let len = arr.length // caching this seems to match V8
        for (let i = 0; i < len; ++i) if (!callbackfn(arr[i], i)) return false
        return true
    }
    ...
    //% shim=String_::substr
    declare function stringSubstrHelper(
        s: string,
        start: number,
        length?: number,
    ): string

    export function stringSubstr(
        s: string,
        start: number,
        length?: number,
    ): string {
    ...
    export function stringTrim(s: string): string {
        ...
        while (start <= end && isWhitespace(s.charCodeAt(start))) ++start

        while (end > start && isWhitespace(s.charCodeAt(end))) --end
