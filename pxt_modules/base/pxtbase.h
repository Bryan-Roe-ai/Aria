...
void registerWithDal(int id, int event, Action a,
                     int flags = 16); // EVENT_LISTENER_DEFAULT_FLAGS
...
#ifdef PXT_VM
inline bool isEncodedDouble(uint64_t v) { return (v >> 48) != 0; }
#endif
...
inline bool isInt(TValue v) { return !isDouble(v) && ((intptr_t)v & 1); }

inline bool isSpecial(TValue v) { return !isDouble(v) && ((intptr_t)v & 2); }

inline bool bothNumbers(TValue a, TValue b) {
  return !isDouble(a) && !isDouble(b) && ((intptr_t)a & (intptr_t)b & 1);
}

inline int numValue(TValue n) { return (int)((intptr_t)n >> 1); }
...
typedef TValue (*RunActionType)(Action a, TValue arg0, TValue arg1,
                                TValue arg2);

#define asmRunAction3 ((RunActionType)(((uintptr_t *)bytecode)[12]))

static inline TValue runAction3(Action a, TValue arg0, TValue arg1,
                                TValue arg2) {
...
static inline TValue runAction0(Action a) { return asmRunAction3(a, 0, 0, 0); }
...
inline TValue incr(TValue e) { return e; }
inline void decr(TValue e) {}

class RefObject;

static inline RefObject *incrRC(RefObject *r) { return r; }
static inline void decrRC(RefObject *) {}

inline void *ptrOfLiteral(int offset) { return &bytecode[offset]; }

// Checks if object is ref-counted, and has a custom PXT vtable in front
// TODO
inline bool isRefCounted(TValue e) { return isPointer(e); }
...
inline void oops(int subcode = 0) { target_panic(800 + subcode); }
...
// assume ARM - ram addresses are 0x2000_0000+; flash is either 0x0+ or
// 0x0800_0000+
#define PXT_IS_READONLY(v) (isTagged(v) || !((uintptr_t)v >> 28))
#endif

inline bool isReadOnly(TValue v) { return PXT_IS_READONLY(v); }
...
public:
  static constexpr ramint_t MaxSize =
      (((1U << (8 * sizeof(ramint_t) - 1)) - 1) << 1) + 1;
...
// A ref-counted collection of either primitive or ref-counted objects (String,
// Image, user-defined record, another collection)
...
typedef TValue (*ActionCB)(TValue *captured, TValue arg0, TValue arg1,
                           TValue arg2);
...
public:
  ...
  inline void stCore(int idx, TValue v) {
    ...
    intcheck(fields[idx] == 0, PANIC_OUT_OF_BOUNDS,
             11); // only one assignment permitted
    fields[idx] = v;
  }
};
...
public:
  ...
  const char *getUTF8DataAt(uint32_t pos) {
    return pos < ascii.length ? ascii.data + pos : NULL;
  }
#endif
...
// Legacy format:
// the first byte of data indicates the format - currently 0xE1 or 0xE4 to 1 or
// 4 bit bitmaps second byte indicates width in pixels third byte indicates the
// height (which should also match the size of the buffer) just like ordinary
// buffers, these can be layed out in flash

// Current format:
// 87 BB WW WW HH HH 00 00 DATA
// that is: 0x87, 0x01 or 0x04 - bpp, width in little endian, height, 0x00, 0x00
// followed by data for 4 bpp images, rows are word-aligned (as in legacy)
...
// 16/20 keeps s.length and s.charCodeAt(i) at about 200 cycles (for actual
// unicode strings), which is similar to amortized allocation time
#define PXT_STRING_SKIP_INCR                                                   \
  16 // needs to be power of 2; needs to be kept in sync with compiler
...
#define PXT_SKIP_DATA_IND(p)                                                   \
  ((const char *)(p->skip.list + PXT_NUM_SKIP_ENTRIES(p)))
#define PXT_SKIP_DATA_PACK(p)                                                  \
  ((const char *)(p->skip_pack.list + PXT_NUM_SKIP_ENTRIES(p)))
...
#define PXT_SHIMS_BEGIN                                                        \
  namespace pxt {                                                              \
  const uintptr_t functionsAndBytecode[] __attribute__((aligned(0x20))) = {    \
      0x08010801, 0x42424242, 0x08010801, 0x8de9d83e, FLASH_TOP,
...
#define DEF_VTABLE_EXT(name, tp, valtype, ...)                                 \
  const VTable name = {                                                        \
      sizeof(tp), valtype, VTABLE_MAGIC2, 0, BuiltInType::tp, BuiltInType::tp, \
      0,          0,       {__VA_ARGS__}};
#else
#define DEF_VTABLE(name, tp, valtype, ...)                                     \
...
#define PXT_VTABLE(classname, valtp)                                           \
  DEF_VTABLE(classname##_vtable, classname, valtp,                             \
             (void *)&classname::destroy, (void *)&classname::print,           \
             (void *)&classname::scan, (void *)&classname::gcsize)

#define PXT_EXT_VTABLE(classname)                                              \
  static int classname##_gcsize() { return sizeof(classname); }                \
  DEF_VTABLE_EXT(classname##_vtable, classname, ValType::Object,               \
                 (void *)&pxt::doNothing, (void *)&pxt::anyPrint,              \
                 (void *)&pxt::doNothing, (void *)&classname##_gcsize)
