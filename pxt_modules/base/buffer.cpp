...
//%
uint8_t *getBytes(Buffer buf) { return buf->data; }
 ...
 */
//%
int getUint8(Buffer buf, int off) { return getByte(buf, off); }

/**
 * Returns false when the buffer can be written to.
 */
//%
bool isReadOnly(Buffer buf) { return buf->isReadOnly(); }

/**
 * Writes an unsigned byte at a particular location
 */
//%
void setUint8(Buffer buf, int off, int v) { setByte(buf, off, v); }

int writeBuffer(Buffer buf, int dstOffset, Buffer src, int srcOffset = 0,
                int length = -1) {
  if (length < 0)
    length = src->length;
...
//% property
int length(Buffer s) { return s->length; }
 ...
 * @param length number of elements in buffer. If negative, length is set as the
 * buffer length minus start. eg: -1
 */
//%
 ...
 */
//%
String toString(Buffer buf) { return mkString((char *)buf->data, buf->length); }
...
void rotate(Buffer buf, int offset, int start = 0, int length = -1) {
  if (length < 0)
    length = buf->length - start;
  if (start < 0 || start > buf->length || length <= 0 ||
      length > buf->length - start || offset == 0 || offset == INT_MIN)
    return;
...
bool BoxedBuffer::isInstance(TValue v) { return getAnyVTable(v) == &buffer_vt; }

// The functions below are deprecated in control namespace, but they are
// referenced in Buffer namespaces via explicit shim=...
namespace control {
/**
 ...
 */
//% deprecated=1
Buffer createBuffer(int size) { return mkBuffer(NULL, size); }
...
namespace pxt {
static int writeBytes(uint8_t *dst, uint8_t *src, int length, bool swapBytes,
                      int szLeft) {
  if (szLeft < length) {
    return -1;
  }
...
static int readBytes(uint8_t *src, uint8_t *dst, int length, bool swapBytes,
                     int szLeft) {
  if (szLeft < length) {
    memset(dst, 0, length);
    return -1;
...
void setNumberCore(uint8_t *buf, int szLeft, NumberFormat format,
                   TNumber value) {
  int8_t i8;
  uint8_t u8;
  int16_t i16;
