...
TValue RefRecord::ld(int idx) {
  // intcheck((reflen == 255 ? 0 : reflen) <= idx && idx < len,
  // PANIC_OUT_OF_BOUNDS, 1);
  return fields[idx];
}
...
//%
void deleteRefObject(RefObject *obj) { obj->destroyVT(); }
...
// this function removes an element at index i and shifts the rest of the
// elements to left to fill the gap
...
// this function inserts element value at index i by shifting the rest of the
// elements right.
void Segment::insert(unsigned i, TValue value) {
  ...
  if (i < length) {
    ...
    // This is insert beyond the length, just call set which will adjust the
    // length
    set(i, value);
  }
#ifdef DEBUG_BUILD
...
void Segment::print() {
  DMESG("Segment: %p, length: %d, size: %d", data, (unsigned)length,
        (unsigned)size);
...
void RefCollection::destroy(RefCollection *t) { t->head.destroy(); }
...
PXT_VTABLE_CTOR(RefRefLocal) { v = 0; }

void RefRefLocal::print(RefRefLocal *t) {
  DMESG("RefRefLocal %p v=%p", t, (void *)t->v);
}

void RefRefLocal::destroy(RefRefLocal *t) { decr(t->v); }
...
int RefMap::findIdx(String key) {
  ...
  for (unsigned i = 0; i < len; ++i) {
    auto s = data[i];
    if (s->getUTF8Size() == keylen &&
        memcmp(keydata, s->getUTF8Data(), keylen) == 0)
      return i;
  }
...
#ifdef PXT_VM
int templateHash() { return *(int *)&vmImg->infoHeader->hexHash; }

int programHash() { return *(int *)&vmImg->infoHeader->programHash; }

int getNumGlobals() { return (int)vmImg->infoHeader->allocGlobals; }

String programName() { return mkString((char *)vmImg->infoHeader->name); }
#else
int templateHash() { return ((int *)bytecode)[4]; }

int programHash() { return ((int *)bytecode)[6]; }

int getNumGlobals() { return bytecode[16]; }

String programName() { return ((String *)bytecode)[15]; }
#endif
...
void start() { exec_binary((unsigned *)functionsAndBytecode); }
#endif
