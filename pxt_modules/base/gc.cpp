...
#define IS_LIVE(vt)                                                            \
  (IS_MARKED(vt) || (((uintptr_t)(vt) >> (HIGH_SHIFT)) == 0x6))
...
//% expose
Buffer getGCStats() { return mkBuffer((uint8_t *)&gcStats, sizeof(gcStats)); }
...
static bool inGCArea(void *ptr) {
  for (auto block = firstBlock; block; block = block->next) {
    if ((void *)block->data <= ptr &&
        ptr < (void *)((uint8_t *)block->data + block->blockSize))
      return true;
  }
  return false;
}
...
static uint32_t getObjectSize(RefObject *o) {
  ...
  GC_CHECK(1 <= r && (r <= BYTES_TO_WORDS(GC_MAX_ALLOC_SIZE) || IS_FREE(vt)),
           41);
  return r;
}
...
static GCBlock *allocateBlockCore() {
  ...
  if (lowMem) {
    ...
    auto memSize =
        ((heapSize + memIncrement - 1) / memIncrement) * memIncrement;
...
void gcFreeze() { inGC |= IN_GC_FREEZE; }
...
#ifdef PXT_VM
void gcPreStartup() { inGC |= IN_GC_PREALLOC; }

void gcStartup() { inGC &= ~IN_GC_PREALLOC; }
#endif
...
void *gcAllocate(int numbytes) {
  ...
  for (int i = 0;; ++i) {
    ...
    for (auto p = firstFree; p; p = p->nextFree) {
      ...
      VVLOG("%p %d - %d = %d", (void *)vt, (int)VAR_BLOCK_WORDS(vt),
            (int)numwords, left);
      if (left >= 0) {
        auto nf = (RefBlock *)((void **)p + numwords);
        auto nextFree =
            p->nextFree; // p and nf can overlap when allocating 4 bytes
    ...
    // GC didn't help, try new block
      ...
      DMESG("gcAlloc(%d) (%d/%d free; %d max block) -> new block", numbytes,
            gcStats.lastFreeBytes, gcStats.totalBytes,
            gcStats.lastMaxBlockBytes);
      allocateBlock();
    } else
...
void RefImage::scan(RefImage *t) { gcScan((TValue)t->buffer); }

void RefCollection::scan(RefCollection *t) { gcScanSegment(t->head); }

void RefAction::scan(RefAction *t) { gcScanMany(t->fields, t->len); }

void RefRefLocal::scan(RefRefLocal *t) { gcScan(t->v); }
...
unsigned RefImage::gcsize(RefImage *t) { return SIZE(0); }

unsigned RefCollection::gcsize(RefCollection *t) { return SIZE(0); }

unsigned RefAction::gcsize(RefAction *t) {
  return SIZE(WORDS_TO_BYTES(t->len));
}

unsigned RefRefLocal::gcsize(RefRefLocal *t) { return SIZE(0); }

unsigned RefMap::gcsize(RefMap *t) { return SIZE(0); }
