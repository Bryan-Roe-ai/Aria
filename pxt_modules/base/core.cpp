...
void coreReset() {
  // these are allocated on GC heap, so they will go away together with the
  // reset
    ...
    if ((c & 0x80) == 0x00) {
      ...
      if (charCode < 0x800 /*|| (0xd800 <= charCode && charCode <= 0xdfff)*/)
        ...
        i += 3;
               ...
               (data[i + 1] & 0xc0) == 0x80 && (data[i + 2] & 0xc0) == 0x80 &&
               (data[i + 3] & 0xc0) == 0x80) {
      charCode = ((c & 0x07) << 18) | (data[i + 1] & 0x3f) << 12 |
...
String mkStringCore(const char *data, int len) {
  ...
  if (data && isUTF8(data, len)) {
    vt = len >= PXT_STRING_MIN_SKIP ? &string_skiplist16_packed_vt
                                    : &string_inline_utf8_vt;
  }
  if (vt == &string_skiplist16_packed_vt) {
    int ulen = utf8Len(data, len);
    r = new (gcAllocate(sizeof(void *) + 2 + 2 +
                        (ulen / PXT_STRING_SKIP_INCR) * 2 + len + 1))
        BoxedString(vt);
...
String mkString(const char *data, int len) {
  ...
  // this could be optimized, but it only kicks in when the string isn't valid
  // utf8 (or we need to introduce surrogate pairs) which is unlikely to be
...
#if PXT_UTF8
// This converts surrogate pairs, which are encoded as 2 characters of 3 bytes
// each into a proper 4 byte utf-8 character.
...
//%
void seedRandom(unsigned seed) { random_value = seed; }

//% expose
void seedAddRandom(unsigned seed) { random_value ^= 0xCA2557CB * seed; }
...
unsigned getRandom(unsigned max) {
  ...
  do {
    ...
    do {
      ...
      // We use an optimal sequence with a period of 2^32-1, as defined by Bruce
      // Schneier here (a true legend in the field!), For those interested, it's
      // documented in his paper: "Pseudo-Random Sequence Generator for 32-Bit
      // CPUs: A fast, machine-independent generator for 32-bit Microprocessors"
      // https://www.schneier.com/paper-pseudorandom-sequence.html
      unsigned r = random_value;

      r = ((((r >> 31) ^ (r >> 6) ^ (r >> 4) ^ (r >> 2) ^ (r >> 1) ^ r) & 1)
           << 31) |
          (r >> 1);
...
//%
String mkEmpty() { return (String)emptyString; }
...
#if PXT_UTF8
  if (IS_CONS(s)) {
    // (s->cons.left + s->cons.right) + other = s->cons.left + (s->cons.right +
    // other)
    ...
    String r =
        new (gcAllocate(3 * sizeof(void *))) BoxedString(&string_cons_vt);
    registerGCObj(r);
...
//%
int length(String s) { return s->getLength(); }

#define isspace(c) ((c) == ' ')
#define iswhitespace(c)                                                        \
  ((c) == 0x09 || (c) == 0x0B || (c) == 0x0C || (c) == 0x20 ||                 \
   (uint8_t)(c) == 0xA0 || (c) == 0x0A || (c) == 0x0D)
...
int toInt(TNumber v) { return (int)toUInt(v); }
...
float toFloat(TNumber v) {
  if (v == TAG_NAN || v == TAG_UNDEFINED)
    return NAN;
  // optimize for the int case - this will avoid software conversion when FPU is
  // present
  if (isTagged(v))
    return toInt(v);
...
//%
TNumber ands(TNumber a, TNumber b) { BITOP(&) }
...
// This cannot be more than 15, as this is the most that can be accurately
// represented in 64 bit double. Otherwise this code may crash.
#define DIGITS 15
...
//%
TNumber random() { return fromDouble(randomDouble()); }
...
TNumber round(TNumber x) {
  // In C++, round(-1.5) == -2, while in JS, round(-1.5) == -1. Align to the JS
  // convention for consistency between simulator and device. The following does
  // rounding with ties (x.5) going towards positive infinity.
...
void push(RefCollection *c, TValue x) { c->head.push(x); }
TValue pop(RefCollection *c) { return c->head.pop(); }
TValue getAt(RefCollection *c, int x) { return c->head.get(x); }
void setAt(RefCollection *c, int x, TValue y) { c->head.set(x, y); }
TValue removeAt(RefCollection *c, int x) { return c->head.remove(x); }
...
#ifdef PXT_VM
unsigned programSize() { return 0; }
#else
//%
unsigned programSize() { return bytecode[17] * 8; }
#endif
...
int *getBootloaderConfigData() { return NULL; }
...
namespace pxtrt {
//%
TValue ldlocRef(RefRefLocal *r) { return r->v; }

//%
void stlocRef(RefRefLocal *r, TValue v) { r->v = v; }
...
// Store a captured local in a closure. It returns the action, so it can be
// chained.
//%
...
//%
void panic(int code) { soft_panic(code); }
...
#define PRIM_VTABLE(name, objectTp, tp, szexpr)                                \
  ...
  DEF_VTABLE(name##_vt, tp, objectTp, (void *)&dtorDoNothing,                  \
             (void *)&anyPrint, 0, (void *)&name##_size)
...
#define STRING_VT(name, fix, scan, gcsize, data, utfsize, length, dataAt)      \
  ...
  DEF_VTABLE(name##_vt, BoxedString, ValType::String, (void *)&dtorDoNothing,  \
             (void *)&anyPrint, (void *)&name##_gcscan,                        \
             (void *)&name##_gcsize, (void *)&name##_data,                     \
             (void *)&name##_utfsize, (void *)&name##_length,                  \
             (void *)&name##_dataAt)
...
STRING_VT(string_inline_ascii, NOOP, NOOP, 2 + p->ascii.length + 1,
          p->ascii.data, p->ascii.length, p->ascii.length,
          idx <= p->ascii.length ? p->ascii.data + idx : NULL)
#if PXT_UTF8
STRING_VT(string_inline_utf8, NOOP, NOOP, 2 + p->utf8.size + 1, p->utf8.data,
          p->utf8.size, utf8Len(p->utf8.data, p->utf8.size),
          utf8Skip(p->utf8.data, p->utf8.size, idx))
STRING_VT(string_skiplist16, NOOP, if (p->skip.list) gcMarkArray(p->skip.list),
          2 * sizeof(void *), PXT_SKIP_DATA_IND(p), p->skip.size,
          p->skip.length, skipLookup(p, idx, 0))
STRING_VT(string_skiplist16_packed, NOOP, NOOP,
          2 + 2 + PXT_NUM_SKIP_ENTRIES(p) * 2 + p->skip.size + 1,
          PXT_SKIP_DATA_PACK(p), p->skip.size, p->skip.length,
          skipLookup(p, idx, 1))
STRING_VT(string_cons, fixCons(p),
          (gcScan((TValue)p->cons.left), gcScan((TValue)p->cons.right)),
          2 * sizeof(void *), PXT_SKIP_DATA_IND(p), p->skip.size,
          p->skip.length, skipLookup(p, idx, 0))
#endif
...
#define pxt_restore_exception_state                                            \
  ((RestoreStateType)(((uintptr_t *)bytecode)[14]))
#endif
...
__attribute__((weak)) int redirectSamples(int16_t *dst, int numsamples,
                                          int samplerate) {
  return 0;
}
