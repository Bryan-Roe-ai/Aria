...
int millis() { return current_time_ms(); }

/**
 * Gets current time in microseconds. Overflows every ~18 minutes.
 */
//%
int micros() { return current_time_us() & 0x3fffffff; }
...
void waitMicros(int micros) { sleep_us(micros); }
...
void runInParallel(Action a) { pxt::runInParallel(a); }

/**
 * Blocks the calling thread until the specified event is raised.
 */
//% help=control/wait-for-event async
//% blockId=control_wait_for_event block="wait for event|from %src|with value
//%value"
void waitForEvent(int src, int value) { pxt::waitForEvent(src, value); }
...
/**
 * Derive a unique, consistent 64-bit serial number of this device from internal
 * data.
 */
//% blockId="control_device_long_serial_number" block="device long serial
//number" weight=9 % help=control/device-long-serial-number
 ...
 */
//%
void dmesgValue(TValue v) { anyPrint(v); }
} // namespace control
