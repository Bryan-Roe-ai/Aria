...
namespace music {
    ...
    export class Melody {
        ...
        static stopAll() {
            if (Melody.playingMelodies) {
                const ms = Melody.playingMelodies.slice(
                    0,
                    Melody.playingMelodies.length,
                )
                ms.forEach(p => p.stop())
            }
        }
        ...
        playUntilDone(volume = 255) {
            ...
            this._player.onPlayFinished = () => {
                if (p == this._player) this.unregisterMelody()
            }
            this.registerMelody()
    ...
    export class MelodyPlayer {
        ...
        play(volume: number) {
            ...
            const addForm = (
                ...
                const envelopeWidth =
                    ms > 0 ? ms : duration * Math.idiv(15000, tempo) + envR
            ...
            while (true) {
                ...
                if (!currNote) {
                    ...
                    if (this.onPlayFinished) this.onPlayFinished()
                    return
                }
                ...
                if (note && hz < 0) {
                    ...
                    hz =
                        freqs.getNumber(NumberFormat.UInt16LE, keyNumber * 2) ||
                        0
                }
                ...
                if (hz < 0) {
                    ...
                    this.queuePlayInstructions(
                        timePos - now,
                        sndInstr.slice(0, sndInstrPtr),
                    )
                    endHz = -1
    ...
    export const magicWand = new Melody(
        "F#6:1-300 G# A# B C7# D# F F# G# A# B:6",
    )
