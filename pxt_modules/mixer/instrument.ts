namespace music.sequencer {
    ...
    export class Track {
        ...
        constructor(
            ...
            this.currentNoteEvent = new NoteEvent(
                this.buf,
                this.noteEventStart + 2,
            )
        }
    ...
    export function renderInstrument(
        ...
        const ampLFOInterval = instrument.ampLFO.amplitude
            ? Math.max(500 / instrument.ampLFO.frequency, 50)
            : 50
        const pitchLFOInterval = instrument.pitchLFO.amplitude
            ? Math.max(500 / instrument.pitchLFO.frequency, 50)
            : 50
        ...
        let nextPETime = instrument.pitchEnvelope.amplitude
            ? instrument.pitchEnvelope.attack
            : totalDuration
        let nextPLTime = instrument.pitchLFO.amplitude
            ? pitchLFOInterval
            : totalDuration
        let nextALTime = instrument.ampLFO.amplitude
            ? ampLFOInterval
            : totalDuration
        ...
        while (time < totalDuration) {
            // Amp envelope
            if (
                nextAETime <= nextPETime &&
                nextAETime <= nextPLTime &&
                nextAETime <= nextALTime
            ) {
                time = nextAETime
                ...
                if (
                    time <
                        instrument.ampEnvelope.attack +
                            instrument.ampEnvelope.decay &&
                    instrument.ampEnvelope.attack +
                        instrument.ampEnvelope.decay <
                        gateLength
                ) {
            ...
            // Pitch envelope
                ...
                nextPETime <= nextPLTime &&
                nextPETime <= nextALTime &&
                ...
                if (
                    time <
                        instrument.pitchEnvelope.attack +
                            instrument.pitchEnvelope.decay &&
                    instrument.pitchEnvelope.attack +
                        instrument.pitchEnvelope.decay <
                        gateLength
                ) {
                ...
                // Otherwise it must be the end of the release
                    ...
                    nextPETime = Math.min(
                        totalDuration,
                        gateLength + instrument.pitchEnvelope.release,
                    )
                }
        ...
        let prevAmp =
            instrumentVolumeAtTime(instrument, gateLength, 0, volume) | 0
        let prevPitch =
            instrumentPitchAtTime(instrument, noteFrequency, gateLength, 0) | 0
        let prevTime = 0
    ...
    export function renderDrumInstrument(
        sound: DrumInstrument,
        volume: number,
    ) {
    ...
    function instrumentPitchAtTime(
        instrument: Instrument,
        noteFrequency: number,
        gateLength: number,
        time: number,
    ...
    function instrumentVolumeAtTime(
        instrument: Instrument,
        gateLength: number,
        time: number,
        maxVolume: number,
        ...
        return (
            ((Math.max(Math.min(mod, instrument.ampEnvelope.amplitude), 0) /
                1024) *
                maxVolume) |
            0
        )
    ...
    function envelopeValueAtTime(
        ...
        if (time > gateLength) {
            ...
            // Did the gate length end before the attack stage finished?
                ...
                const height =
                    (envelope.amplitude / envelope.attack) * gateLength
                return (
                    height - (height / envelope.release) * (time - gateLength)
                )
            }
            // Did the gate length end before the decay stage finished?
                ...
                const height2 =
                    envelope.amplitude -
                    ((envelope.amplitude - adjustedSustain) / envelope.decay) *
                        (gateLength - envelope.attack)
                return (
                    height2 - (height2 / envelope.release) * (time - gateLength)
                )
            } else {
                return (
                    adjustedSustain -
                    (adjustedSustain / envelope.release) * (time - gateLength)
                )
            }
        } else if (time < envelope.attack) {
    ...
    function lfoValueAtTime(lfo: LFO, time: number) {
        ...
        return (
            Math.cos((time / 1000) * lfo.frequency * 2 * Math.PI) *
            lfo.amplitude
        )
    }
