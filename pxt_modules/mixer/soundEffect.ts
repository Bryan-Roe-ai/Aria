...
namespace music {
    ...
    export function soundToInstructionBuffer(
        waveShape: WaveShape,
        startFrequency: number,
        endFrequency: number,
        startVolume: number,
        ...
        if (
            ...
            const getVolumeAt = (t: number) =>
                ((startVolume + (t * (endVolume - startVolume)) / duration) /
                    255) *
                globalVolume
            ...
            switch (interpolation) {
                case InterpolationCurve.Linear:
                    getFrequencyAt = t =>
                        startFrequency +
                        (t * (endFrequency - startFrequency)) / duration
                    break
                case InterpolationCurve.Curve:
                    getFrequencyAt = t =>
                        startFrequency +
                        (endFrequency - startFrequency) *
                            Math.sin((t / duration) * (Math.PI / 2))
                    break
                case InterpolationCurve.Logarithmic:
                    getFrequencyAt = t =>
                        startFrequency +
                        (Math.log(1 + 9 * (t / duration)) / Math.log(10)) *
                            (endFrequency - startFrequency)
                    break
            }
            ...
            for (let i = 0; i < fxSteps; i++) {
                ...
                if (effect === SoundExpressionEffect.Tremolo) {
                    if (i % 2 === 0) {
                        newStep.volume = Math.max(
                            newStep.volume - fxRange * 500,
                            0,
                        )
                    } else {
                        newStep.volume = Math.min(
                            newStep.volume + fxRange * 500,
                            1023,
                        )
                    }
                    ...
                    if (i % 2 === 0) {
                        newStep.frequency = Math.max(
                            newStep.frequency - fxRange * 100,
                            0,
                        )
                    } else {
                    ...
                    if (i % 2 === 0) {
                        newStep.frequency = Math.max(
                            newStep.frequency - fxRange * 1000,
                            0,
                        )
                    } else {
    ...
    export function randomizeSound(sound: SoundEffect) {
        ...
        if (res.waveShape === WaveShape.Noise) {
            ...
            if (
                res.effect !== SoundExpressionEffect.None ||
                Math.random() < 0.1
            ) {
                res.effect = randomEffect()
            }
        ...
        let newFrequencyDifference =
            oldFrequencyDifference +
            oldFrequencyDifference * 2 * (Math.random() - 0.5)

        if (
            Math.sign(oldFrequencyDifference) !==
            Math.sign(newFrequencyDifference)
        ) {
        ...
        res.endFrequency = Math.clamp(
            1,
            5000,
            res.startFrequency + newFrequencyDifference,
        )

        // Same strategy for volume
        const oldVolumeDifference = res.endVolume - res.startVolume
        let newVolumeDifference =
            oldVolumeDifference + oldVolumeDifference * (Math.random() - 0.5)
        ...
        res.endVolume = Math.clamp(
            0,
            255,
            res.startVolume + newVolumeDifference,
        )
    ...
    function randomEffect() {
        switch (Math.randomRange(0, 2)) {
            case 1:
                return SoundExpressionEffect.Warble
            case 2:
                return SoundExpressionEffect.Tremolo
            case 0:
            default:
    ...
    function randomInterpolation() {
        switch (Math.randomRange(0, 2)) {
            ...
            case 2:
                return InterpolationCurve.Curve
            case 0:
            default:
