...
static int triangleTone(PlayingSound *sound, uint32_t position, uint8_t cycle) {
  (void)sound;
  (void)cycle;
  return position < 512 ? (position << 7) - 0x7fff
                        : ((1023 - position) << 7) - 0x7fff;
}

static int squareWaveTone(PlayingSound *sound, uint32_t position,
                          uint8_t cycle) {
  (void)cycle;
...
static int tunedNoiseTone(PlayingSound *sound, uint32_t position,
                          uint8_t cycle) {
  // Generate a square wave filtered by a random bit sequence. Since the
  // generator is called multiple times per wave, use PlayingSound state data to
  // ensure we only generate a random bit once per wave, and then reuse it for
  // future calls for that wave.
  //
...
// and should avoid long identical-bit runs for the lower parts. The values
// below were chosen based on a random permutation of the hex nibbles 0..f and
// then hand-tweaked by swapping some nibbles. Generated by:
//
...
static int cycleNoiseTone(PlayingSound *sound, uint32_t position,
                          uint8_t cycle) {
...
int WSynthesizer::updateQueues() {
  ...
  while (1) {
    ...
    for (p = waiting; p; p = p->next) {
      int timeLeft = p->state == SoundState::Waiting
                         ? p->startSampleNo - currSample
                         : maxTime;
      if (timeLeft <= 0) {
        break;
      }
    ...
    if (p) {
      ...
      for (unsigned i = 0; i < MAX_SOUNDS; ++i) {
        ...
        if (snd->sound == NULL)
          break;
        if (minIdx == -1 || playingSounds[minIdx].startSampleNo <
                                playingSounds[i].startSampleNo)
          minIdx = i;
        snd = NULL;
      }
      ...
      snd->instrEnd =
          snd->currInstr + p->instructions->length / sizeof(SoundInstruction);
      snd->prevVolume = -1;
    } else {
  ...
  for (unsigned i = 0; i < MAX_SOUNDS; ++i) {
    ...
    for (int j = 0; j < numsamples; ++j) {
      if (samplesLeft == 0) {
        ...
        volumeStep = (int)((instr->endVolume - instr->startVolume) << 16) /
                     (int)samplesLeft;
        ...
        if (j == 0 && snd->prevVolume != -1) {
          ...
          if (instr->frequency != instr->endFrequency) {
            uint32_t endToneStep =
                (uint32_t)(toneStepMult * instr->endFrequency);
            toneDelta =
                (int32_t)(endToneStep - toneStep) / (int32_t)samplesLeft;
          } else {
            toneDelta = 0;
          }
        }
...
//% promise
int _createSequencer() { return 0; }

//%
String _sequencerState(int id) { return NULL; }

//%
int _sequencerCurrentTick(int id) { return 0; }
...
//%
void _sequencerSetDrumTrackVolume(int id, int trackIndex, int drumIndex,
                                  int volume) {}
