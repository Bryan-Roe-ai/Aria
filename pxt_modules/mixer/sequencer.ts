namespace music.sequencer {
    ...
    export class Sequencer {
        ...
        start(loop: boolean) {
            ...
            control.runInParallel(() => {
                while (this.isPlaying) {
                    ...
                    if (
                        this.currentTick >=
                        this.song.beatsPerMeasure *
                            this.song.measures *
                            this.song.ticksPerBeat
                    ) {
        ...
        tickToMs(ticks: number) {
            return (
                (60000 / this.song.beatsPerMinute / this.song.ticksPerBeat) *
                ticks
            )
        }
            ...
            for (let i = 0; i < track.currentNoteEvent.polyphony; i++) {
                playInstructions(
                    ...
                    renderInstrument(
                        track.instrument,
                        lookupFrequency(
                            track.currentNoteEvent.getNote(
                                i,
                                track.instrument.octave,
                            ),
                        ),
                        this.tickToMs(
            ...
            for (let i = 0; i < track.currentNoteEvent.polyphony; i++) {
                playInstructions(
                    ...
                    renderDrumInstrument(
                        track.drums[
                            track.currentNoteEvent.getNote(i, undefined)
                        ],
                        music.volume(),
                    ),
                )
    ...
    declare function _sequencerPlaySong(
        id: number,
        song: Buffer,
        loop: boolean,
    ): void
    ...
    declare function _sequencerSetTrackVolume(
        id: number,
        trackIndex: number,
        volume: number,
    ): void
    ...
    declare function _sequencerSetDrumTrackVolume(
        id: number,
        trackIndex: number,
        drumIndex: number,
        volume: number,
