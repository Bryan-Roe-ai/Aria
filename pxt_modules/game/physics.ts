...
const SPRITE_NO_TILE_OVERLAPS =
    SpriteFlag.GhostThroughTiles |
    sprites.Flag.Destroyed |
    SpriteFlag.RelativeToCamera
const SPRITE_NO_WALL_COLLISION =
    SpriteFlag.GhostThroughWalls |
    sprites.Flag.IsClipping |
    sprites.Flag.Destroyed |
    SpriteFlag.RelativeToCamera
const SPRITE_NO_SPRITE_OVERLAPS =
...
class ArcadePhysicsEngine extends PhysicsEngine {
    ...
    move(dt: number) {
        ...
        const movingSprites = this.sprites.map(sprite =>
            this.createMovingSprite(sprite, dtMs, dt2),
        )
        ...
        const MAX_STEP_COUNT = Fx.toInt(
            Fx.idiv(
                Fx.imul(Fx.div(this.maxVelocity, this.minSingleStep), dtMs),
                1000,
            ),
        )
        ...
        for (
            let count = 0;
            count < MAX_STEP_COUNT && buffers[selected].length !== 0;
            ++count
        ) {
            ...
            for (let ms of currMovers) {
                ...
                const stepX =
                    Fx.abs(ms.xStep) > Fx.abs(ms.dx) ? ms.dx : ms.xStep
                const stepY =
                    Fx.abs(ms.yStep) > Fx.abs(ms.dy) ? ms.dy : ms.yStep
                ...
                if (
                    !(s.flags & SPRITE_NO_SPRITE_OVERLAPS) &&
                    s._kindsOverlappedWith.length
                ) {
                ...
                if (
                    s.flags & sprites.Flag.StayInScreen ||
                    (bounce && !tileMap)
                ) {
                ...
                if (
                    Fx.abs(ms.dx) > MIN_MOVE_GAP ||
                    Fx.abs(ms.dy) > MIN_MOVE_GAP
                ) {
    ...
    protected createMovingSprite(
        ...
        if (sprite._ax) {
            sprite._vx = Fx.add(
                sprite._vx,
                Fx.idiv(Fx.imul(sprite._ax, dtMs), 1000),
            )
        } else if (sprite._fx) {
            const fx = Fx.idiv(Fx.imul(sprite._fx, dtMs), 1000)
            ...
            else sprite._vx = Fx.zeroFx8
        }

        if (sprite._ay) {
            sprite._vy = Fx.add(
                sprite._vy,
                Fx.idiv(Fx.imul(sprite._ay, dtMs), 1000),
            )
        } else if (sprite._fy) {
            const fy = Fx.idiv(Fx.imul(sprite._fy, dtMs), 1000)
        ...
        while (
            Fx.abs(xStep) > this.maxSingleStep ||
            Fx.abs(yStep) > this.maxSingleStep
        ) {
        ...
        for (const bucket of this.map.filledBuckets) {
            ...
            for (const sprite of bucket) {
                ...
                for (const overlapper of bucket) {
                    ...
                    const higher =
                        sprite.id > overlapper.id ? sprite : overlapper
                    ...
                    if (higher._alreadyChecked.indexOf(lower.id) !== -1)
                        continue
                    ...
                    if (sprite._kindsOverlappedWith.indexOf(otherKind) === -1)
                        continue
                    ...
                    for (const h of handlers) {
                        if (
                            ...
                            control.runInParallel(() => {
                                if (
                                    !(
                                        (sprite.flags | overlapper.flags) &
                                        SPRITE_NO_SPRITE_OVERLAPS
                                    )
                                ) {
                                    if (thisKind === h.kind) {
    ...
    protected screenEdgeCollisions(
        ...
        if (offset < 0) {
            ...
            (offset =
                Fx.toFloat(s._hitbox.right) - camera.offsetX - screen.width) > 0
        ) {
            s.right -= offset
            if (bounce) s.vx = -s.vx
        }
        if ((offset = Fx.toFloat(s._hitbox.top) - camera.offsetY) < 0) {
            ...
            (offset =
                Fx.toFloat(s._hitbox.bottom) - camera.offsetY - screen.height) >
            0
        ) {
            s.bottom -= offset
            if (bounce) s.vy = -s.vy
    ...
    protected tilemapCollisions(movingSprite: MovingSprite, tm: tiles.TileMap) {
        ...
        const xDiff = Fx.sub(s._x, s._lastX)

        const yDiff = Fx.sub(s._y, s._lastY)
        ...
        if (!(s.flags & SPRITE_NO_WALL_COLLISION)) {
            if (xDiff !== Fx.zeroFx8) {
                ...
                for (
                    ...
                    const y0 = Fx.toIntShifted(
                        Fx.add(
                            Fx.min(y, Fx.sub(hbox.bottom, yDiff)),
                            Fx.oneHalfFx8,
                        ),
                        tileScale,
                    )
                    ...
                    if (tm.isObstacle(x0, y0)) {
                        ...
                        if (
                            !collidedTiles.some(
                                o => o.tileIndex === obstacle.tileIndex,
                            )
                        ) {
                ...
                if (collidedTiles.length) {
                    const collisionDirection = right
                        ? CollisionDirection.Right
                        : CollisionDirection.Left
                    s._x = Fx.sub(
                        right
                            ? Fx.sub(Fx8(x0 << tileScale), hbox.width)
                            : Fx8((x0 + 1) << tileScale),
                        hbox.ox,
                    )
                    ...
                    if (s.flags & sprites.Flag.DestroyOnWall) {
                        ...
                        s._vx === movingSprite.cachedVx &&
                        !(s.flags & SPRITE_NO_WALL_COLLISION)
                    ) {
                        ...
                        Math.sign(Fx.toInt(s._vx)) ===
                        Math.sign(Fx.toInt(movingSprite.cachedVx))
                    ) {
            ...
            if (yDiff !== Fx.zeroFx8) {
                ...
                for (
                    ...
                    const x0 = Fx.toIntShifted(
                        Fx.add(Fx.min(x, hbox.right), Fx.oneHalfFx8),
                        tileScale,
                    )
                ...
                if (collidedTiles.length) {
                    const collisionDirection = down
                        ? CollisionDirection.Bottom
                        : CollisionDirection.Top
                    s._y = Fx.sub(
                        down
                            ? Fx.sub(Fx8(y0 << tileScale), hbox.height)
                            : Fx8((y0 + 1) << tileScale),
                        hbox.oy,
                    )
                    ...
                    if (s.flags & sprites.Flag.DestroyOnWall) {
                        ...
                        Math.sign(Fx.toInt(s._vy)) ===
                        Math.sign(Fx.toInt(movingSprite.cachedVy))
                    ) {
                        // sprite collision event changed velocity,
                        // but still facing same direction; prevent further movement this update.
                        movingSprite.dy = Fx.zeroFx8
        ...
        if (!(s.flags & SPRITE_NO_TILE_OVERLAPS)) {
            ...
            for (
                ...
                for (
                    ...
                    const y0 = Fx.toIntShifted(
                        Fx.add(Fx.min(y, hbox.bottom), Fx.oneHalfFx8),
                        tileScale,
                    )
                    ...
                    if (
                        !tm.isObstacle(x0, y0) ||
                        !!(s.flags & sprites.Flag.GhostThroughWalls)
                    ) {
     ...
     */
    protected tilemapOverlaps(
        sprite: Sprite,
        overlappedTiles: tiles.Location[],
    ) {
        ...
        for (const tile of overlappedTiles) {
            if (
                alreadyHandled.some(
                    l => l.column === tile.column && l.row === tile.row,
                )
            ) {
                continue
            }
            ...
            if (tileOverlapHandlers) {
                tileOverlapHandlers
                    .filter(
                        h =>
                            h.spriteKind == sprite.kind() &&
                            h.tileKind.equals(tiles.getTileImage(tile)),
                    )
    ...
    public moveSprite(s: Sprite, dx: Fx8, dy: Fx8) {
        ...
        if (tm && tm.enabled) {
            ...
            if (
                Math.abs(Fx.toInt(dx)) <= maxDist &&
                Math.abs(Fx.toInt(dy)) <= maxDist
            ) {
                const ms = new MovingSprite(s, s._vx, s._vy, dx, dy, dx, dy)
    ...
    protected constrain(v: Fx8) {
        return Fx.max(Fx.min(this.maxVelocity, v), this.maxNegativeVelocity)
    }
}
