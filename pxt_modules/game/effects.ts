namespace effects {
    ...
    export class ImageEffect implements BackgroundEffect {
        ...
        constructor(
            defaultRate: number,
            effectFactory: (image: Image, fastRandom?: Math.FastRandom) => void,
        ) {
    ...
    export const dissolve = new ImageEffect(
        100,
        (input: Image, r: Math.FastRandom) => {
    ...
    export const melt = new ImageEffect(
        125,
        (input: Image, r: Math.FastRandom) => {
    ...
    export const slash = new ImageEffect(
        125,
        (input: Image, r: Math.FastRandom) => {
            const rounds = 12
            for (let j = 0; j < rounds; ++j) {
                ...
                let x = r.randomRange(
                    0,
                    input.width - (horizontal ? length : 1),
                )
                let y = r.randomRange(
                    0,
                    input.height - (horizontal ? 3 : length),
                )
                input.drawLine(
                    x,
                    y,
                    horizontal ? x + length : x,
                    horizontal ? y : y + length,
                    1,
                )
            }
        },
    ...
    export const splatter = new ImageEffect(
        125,
        (input: Image, r: Math.FastRandom) => {
            const imgs: Image[] = [
                img`
