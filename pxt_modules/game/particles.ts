    ...
    export class ParticleSource extends sprites.BaseSprite {
        ...
        __draw(camera: scene.Camera) {
            ...
            const left =
                this.pFlags & Flag.relativeToCamera
                    ? Fx.zeroFx8
                    : Fx8(camera.drawOffsetX)
            const top =
                this.pFlags & Flag.relativeToCamera
                    ? Fx.zeroFx8
                    : Fx8(camera.drawOffsetY)
        ...
        _update(dt: number) {
            ...
            if (this.lifespan !== undefined) {
                ...
                this.anchor &&
                this.anchor.flags !== undefined &&
                this.anchor.flags & sprites.Flag.Destroyed
            ) {
                this.lifespan = 750
            }
        ...
        _prune() {
            ...
            if (this.pFlags & Flag.destroyed && !this.head) {
                ...
                if (scene) scene.allSprites.removeElement(this)
                const sources = particleSources()
        ...
        protected drawParticle(p: Particle, screenLeft: Fx8, screenTop: Fx8) {
            this._factory.drawParticle(
                p,
                Fx.sub(p._x, screenLeft),
                Fx.sub(p._y, screenTop),
            )
        }
    }
