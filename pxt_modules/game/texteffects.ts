namespace texteffects {
    ...
    //% fixedInstance whenUsed
    export const wave = new TextEffect(
        (index?: number, state?: WaveEffectState) => {
            // TODO cast state type
            if (!state) {
                state = {
                    xOffset: 0,
    ...
    export class TextSprite {
        ...
        updateState() {
            ...
            while (i < this._str.length) {
                this._state[i] = this._effect.getState(
                    i,
                    i < this._state.length ? this._state[i] : null,
                )
                i++
            }
        }
        ...
        draw(
            ...
            let s = this._str.substr(
                Math.max(start, 0),
                Math.min(length, this._str.length - start),
            )
