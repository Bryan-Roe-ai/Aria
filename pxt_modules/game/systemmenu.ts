namespace scene.systemMenu {
    ...
    export class MenuOption {
        ...
        show() {
            this.card = sprites.create(
                this.theme ? this.theme.basicCard : CARD_NORMAL,
            )
            this.card.z = 1
        ...
        protected updateCard() {
            ...
            switch (this.state) {
                case CardState.None:
                    this.card.setImage(this.theme.basicCard)
                    break
                case CardState.Selected:
                    this.card.setImage(this.theme.selectedCard)
                    break
                case CardState.Active:
                    this.card.setImage(this.theme.activeCard)
                    break
            }
    ...
    export class PauseMenu {
        ...
        show() {
            ...
            for (let i = 0; i < this.options.length; i++) {
                ...
                current.position(
                    this.theme.padding +
                        (i % this.theme.cardsPerRow) *
                            (this.theme.cardWidth + this.theme.cardSpacing),
                    this.theme.cardsTop +
                        Math.idiv(i, this.theme.cardsPerRow) *
                            (this.theme.cardWidth + this.theme.cardSpacing),
                )
            }
            ...
            controller.up.onEvent(SYSTEM_KEY_DOWN, () => {
                this.setSelection(
                    Math.max(0, this.selection - this.theme.cardsPerRow),
                )
            })
            ...
            controller.down.onEvent(SYSTEM_KEY_DOWN, () => {
                this.setSelection(
                    Math.min(
                        ...
                        this.selection + this.theme.cardsPerRow,
                    ),
                )
            })
            ...
            controller.right.onEvent(SYSTEM_KEY_DOWN, () => {
                this.setSelection(
                    Math.min(this.options.length - 1, this.selection + 1),
                )
            })
        ...
        onUpdate() {
            ...
            for (let i = 0; i < this.options.length; i++) {
                this.options[i].setOffset(
                    2 *
                        Math.sin(
                            t - (i % this.theme.cardsPerRow) * (Math.PI / 2),
                        ),
                )
            }
        ...
        setSelection(selection: number) {
            ...
            if (this.options[this.selection]) {
                this.options[this.selection].setState(
        ...
        drawText() {
            ...
            // Header text
            screen.printCenter(
                this.theme.headerText,
                2,
                1,
                this.theme.headerFont,
            )

            // Black bar for the info box to draw on
            screen.fillRect(
                0,
                this.theme.infoTop - 3,
                screen.width,
                screen.height - this.theme.infoTop + 6,
                15,
            )
            ...
            screen.fillRect(
                this.theme.padding,
                this.theme.infoTop,
                screen.width - (this.theme.padding << 1),
                this.theme.infoFont.charHeight + 1,
                1,
            )

            // Info text
            screen.printCenter(
                this.options[this.selection].getText(),
                this.theme.infoTop + 1,
                15,
                this.theme.infoFont,
            )
        }
        ...
        protected updateScrollTarget() {
            ...
            this.scrollTarget =
                this.scrollRow * (this.theme.cardSpacing + this.theme.cardWidth)
        }
    }
    ...
    export function buildOptionList(): MenuOption[] {
        ...
        if (setScreenBrightnessSupported()) {
            options.push(
                new MenuOption(
                    ...
                    () => `BRIGHTNESS DOWN (${screen.brightness()})`,
                    brightnessDown,
                ),
            )
            options.push(
                new MenuOption(
                    BRIGHTNESS_UP_ICON,
                    () => `BRIGHTNESS UP (${screen.brightness()})`,
                    brightnessUp,
                ),
            )
        }
        options.push(
            new MenuOption(
                STATS_ICON,
                () => (game.stats ? "HIDE STATS" : "SHOW STATS"),
                toggleStats,
            ),
        )
        options.push(
    ...
    function initVolume() {
        ...
        if (vol !== undefined) music.setVolume(vol)
    }

    function initScreen() {
        const brightness = settings.readNumber("#brightness")
        if (brightness !== undefined) screen.setBrightness(brightness)
    }
