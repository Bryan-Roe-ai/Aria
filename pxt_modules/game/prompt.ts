namespace game {
    ...
    const ROW_LEFT =
        PADDING +
        Math.floor((CONTENT_WIDTH - CELL_WIDTH * ALPHABET_ROW_LENGTH) / 2)
    ...
    //% whenUsed=true
    const BOTTOM_BAR_SHIFT_X =
        (BOTTOM_BAR_BUTTON_WIDTH - font.charWidth * 3) / 2
    //% whenUsed=true
    const BOTTOM_BAR_CONFIRM_X =
        (BOTTOM_BAR_BUTTON_WIDTH - font.charWidth * 2) / 2
    //% whenUsed=true
    ...
    //% whenUsed=true
    const ALPHABET_TOP =
        CONTENT_TOP + CONTENT_HEIGHT - ALPHABET_HEIGHT - BOTTOM_BAR_HEIGHT
    //% whenUsed=true
    ...
    export class Prompt {
        ...
        show(
            ...
            if (
                !useOnScreenKeyboard &&
                control.deviceDalVersion() === "sim" &&
                helpers._isSystemKeyboardSupported()
            ) {
                ...
                control.onEvent(_KEYBOARD_CHANGE_EVENT, 0, () => {
                    this.result = helpers
                        ._getTextPromptString()
                        .substr(0, this.answerLength)
        ...
        protected createRenderable() {
            ...
            const promptText = new sprites.RenderText(
                this.message,
                CONTENT_WIDTH,
            )
            ...
            this.renderable = scene.createRenderable(-1, () => {
                promptText.draw(
                    screen,
                    (screen.width >> 1) - (promptText.width >> 1),
                    CONTENT_TOP,
                    this.theme.colorPrompt,
                    0,
                    2,
                )
                this.drawInputArea()
                ...
                if (!systemKeyboardText) {
                    systemKeyboardText = new sprites.RenderText(
                        helpers._getLocalizedInstructions(),
                        CONTENT_WIDTH,
                    )
                }
                ...
                screen.fillRect(
                    0,
                    screen.height - (PADDING << 1) - systemKeyboardText.height,
                    screen.width,
                    screen.height,
                    this.theme.colorBottomBackground,
                )
                systemKeyboardText.draw(
                    screen,
                    PADDING,
                    screen.height - PADDING - systemKeyboardText.height,
        ...
        protected drawInputArea() {
            const answerLeft =
                ...
                Math.floor(
                    (CELL_WIDTH * ALPHABET_ROW_LENGTH -
                        CELL_WIDTH *
                            Math.min(this.answerLength, ALPHABET_ROW_LENGTH)) /
                        2,
                )
            ...
            for (let i = 0; i < this.answerLength; i++) {
                ...
                screen.fillRect(
                    ...
                    1,
                    !this.useSystemKeyboard &&
                        !this.blink() &&
                        i === this.selectionStart
                        ? this.theme.colorInputHighlighted
                        : this.theme.colorInput,
                )
            ...
            if (this.useSystemKeyboard) {
                if (
                    this.selectionStart === this.selectionEnd &&
                    this.selectionStart < this.answerLength
                ) {
                    const col = this.selectionStart % ALPHABET_ROW_LENGTH
                    const row = Math.floor(
                        this.selectionStart / ALPHABET_ROW_LENGTH,
                    )
                    if (!this.blink()) {
                        screen.fillRect(
                            answerLeft + col * CELL_WIDTH,
        ...
        protected drawKeyboard() {
            const top =
                screen.height -
                BOTTOM_BAR_HEIGHT -
                this.keyboardRows * CELL_HEIGHT -
                PADDING
            const left =
                (screen.width >> 1) - ((CELL_WIDTH * this.keyboardColumns) >> 1)
        ...
        protected moveHorizontal(right: boolean) {
            if (right) {
                this.cursorColumn =
                    (this.cursorColumn + 1) % this.keyboardColumns
            } else {
                this.cursorColumn =
                    (this.cursorColumn + (this.keyboardColumns - 1)) %
                    this.keyboardColumns
            }
        }
        ...
        protected confirm() {
            if (this.cursorRow === 3) {
                ...
                const index =
                    this.cursorColumn + this.cursorRow * this.keyboardColumns
