...
namespace controller {
    ...
    declare function pressureLevelByButtonId(
        btnId: number,
        codalId: number,
    ): number
    ...
    export class Button {
        ...
        constructor(id: number, configKey: number) {
            ...
            if (id > 0) {
                ...
                if (configKey > 0) setupButton(id, configKey)
            }
        }

        private raiseButtonUp() {
            if (_userEventsEnabled) control.raiseEvent(KEY_UP, this.id)
            else control.raiseEvent(SYSTEM_KEY_UP, this.id)
        }

        private raiseButtonDown() {
            if (_userEventsEnabled) control.raiseEvent(KEY_DOWN, this.id)
            else control.raiseEvent(SYSTEM_KEY_DOWN, this.id)
        }

        private raiseButtonRepeat() {
            if (_userEventsEnabled) control.raiseEvent(KEY_REPEAT, this.id)
            else control.raiseEvent(SYSTEM_KEY_REPEAT, this.id)
        }
        ...
        removeEventListener(event: ControllerButtonEvent, handler: () => void) {
            ...
            for (let i = 0; i < handlerState.system.length; i++) {
                if (
                    handlerState.system[i].event === event &&
                    handlerState.system[i].callback === handler
                ) {
        ...
        setPressed(pressed: boolean) {
            if (this._pressed != pressed) {
                power.poke()
                if (this._owner) this._owner.connected = true
        ...
        __update(dtms: number) {
            ...
            // inital delay
            if (this._pressedElasped < delay) return

            // repeat count for this step
            const count = Math.floor(
                (this._pressedElasped - delay - interval) / interval,
            )
        ...
        protected runButtonEvents(event: ControllerButtonEvent) {
            ...
            if (handlerState.system) {
                for (const eventHandler of handlerState.system) {
                    if (eventHandler.event === event && eventHandler.callback)
        ...
        protected getOrCreateHandlerForEvent(event: ControllerButtonEvent) {
            if (!this.handlerState) {
                game.currentScene().buttonEventHandlers.push(
                    new ButtonEventHandlerState(this.id),
                )
            }
