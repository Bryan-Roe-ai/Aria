namespace sprites {
    ...
    export class SpriteSayRenderer extends BaseSpriteSayRenderer {
        static drawSayFrame(
            ...
            if (
                textLeft + textWidth < 0 ||
                textTop + textHeight < 0 ||
                textLeft > canvas.width ||
                textTop > canvas.height
            )
                return
            ...
            if (textHeight) {
                ...
                if (
                    speakerX > textLeft &&
                    speakerX < textLeft + textWidth &&
                    speakerY > textTop &&
                    speakerY < textTop + textHeight
                )
                    return
                ...
                if (xDiff > yDiff) {
                    if (speakerX > textLeft + textWidth) {
                        const anchorY = Math.max(
                            Math.min(speakerY, textTop + textHeight - 4),
                            textTop + 5,
                        )
                        canvas.fillRect(
                        ...
                        const anchorX = Math.max(
                            Math.min(speakerX, textLeft + textWidth - 4),
                            textLeft + 5,
                        )
                        canvas.fillRect(
                            anchorX - 2,
        ...
        constructor(
            ...
            if (animated) {
                this.animation = new sprites.RenderTextAnimation(
                    this.renderText,
                    40,
                )
                if (timeOnScreen >= 0) {
                    ...
                    const pauseTime = Math.min(
                        (timeOnScreen / (2 * numberOfPauses)) | 0,
                        1000,
                    )
                    this.animation.setPauseLength(pauseTime)
                    this.animation.setTextSpeed(
                        (this.renderText.printableCharacters() * 1000) /
                            (timeOnScreen - pauseTime * numberOfPauses),
                    )
                }
        ...
        draw(screen: Image, camera: scene.Camera, owner: Sprite) {
            const ox =
                owner.flags & sprites.Flag.RelativeToCamera
                    ? 0
                    : camera.drawOffsetX
            const oy =
                owner.flags & sprites.Flag.RelativeToCamera
            ...
            const height = this.animation
                ? this.animation.currentHeight()
                : this.renderText.height
            const width = this.animation
                ? this.animation.currentWidth()
            ...
            if (
                sayLeft + width < 0 ||
                sayTop + height < 0 ||
                sayLeft > screen.width ||
                sayTop > screen.height
            )
                return

            SpriteSayRenderer.drawSayFrame(
                sayLeft,
                sayTop,
                width,
                height,
    ...
    export class LegacySpriteSayRenderer extends BaseSpriteSayRenderer {
        ...
        constructor(
            ...
            let startY = 2
            let bubbleWidth =
                textToDisplay.length * font.charWidth + bubblePadding
            ...
            if (
                ...
                if (this.sayBubbleSprite)
                    // sprite with same image size, we can reuse it
            ...
            this.updateSay = (dt, camera) => {
                ...
                this.sayBubbleSprite.y =
                    owner.top +
                    bubbleOffset -
                    ((font.charHeight + bubblePadding) >> 1) -
                    2
                ...
                if (!timeOnScreen || timeOnScreen > game.runtime()) {
                    ...
                    if (!owner.isOutOfScreen(camera)) {
                        ...
                        if (this.sayBubbleSprite.top - oy < 0) {
                            this.sayBubbleSprite.y =
                                (this.sayBubbleSprite.y - 2 * owner.y) * -1
                        }
                    }
                    ...
                    if (needsRedraw) {
                        ...
                        if (maxOffset < 0) {
                            this.sayBubbleSprite.image.print(
                                textToDisplay,
                                startX,
                                startY,
                                this.fgColor,
                        ...
                        // Left side padding
                        this.sayBubbleSprite.image.fillRect(
                            0,
                            0,
                            bubblePadding >> 1,
                            font.charHeight + bubblePadding,
                            this.bgColor,
                        )
                        // Right side padding
                        this.sayBubbleSprite.image.fillRect(
                            bubbleWidth - (bubblePadding >> 1),
                            0,
                            bubblePadding >> 1,
                            font.charHeight + bubblePadding,
                            this.bgColor,
                        ...
                        this.sayBubbleSprite.image.setPixel(
                            bubbleWidth - 1,
                            0,
                            0,
                        )
        ...
        update(dt: number, camera: scene.Camera, owner: Sprite) {
            ...
            this.sayBubbleSprite.setFlag(
                SpriteFlag.RelativeToCamera,
                !!(owner.flags & SpriteFlag.RelativeToCamera),
            )
