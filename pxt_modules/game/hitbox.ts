namespace game {
    export class Hitbox {
        ...
        get right() {
            return Fx.sub(Fx.add(this.width, this.left), Fx.oneFx8)
        }

        get bottom() {
            return Fx.sub(Fx.add(this.height, this.top), Fx.oneFx8)
        }
    ...
    export function calculateHitBox(s: Sprite): Hitbox {
        if (s._hitbox && s._hitbox.isValid()) return s._hitbox

        if (s._rotatedBBox) {
            return new Hitbox(
                s,
                Fx8(s._rotatedBBox.width),
                Fx8(s._rotatedBBox.height),
                Fx.zeroFx8,
                Fx.zeroFx8,
            )
        }
        ...
        for (
            let c = 0, fxc = Fx.zeroFx8;
            c < i.width;
            c++, fxc = Fx.add(fxc, Fx.oneFx8)
        ) {
            for (
                let r = 0, fxr = Fx.zeroFx8;
                r < i.height;
                r++, fxr = Fx.add(fxr, Fx.oneFx8)
            ) {
                if (i.getPixel(c, r)) {
