...
public:
  ...
  AnalogButton(AnalogCache *cache, uint16_t id, int threshold)
      : PressureButton(*cache->pin, id), cache(cache), threshold(threshold),
        state(false) {}
...
void setupButton(int buttonId, int key) {
  ...
  if (btn == NULL) {
    ...
    EventModel::defaultEventBus->listen(btn->id, DEVICE_BUTTON_EVT_DOWN,
                                        sendBtnDown);
    EventModel::defaultEventBus->listen(btn->id, DEVICE_BUTTON_EVT_UP,
                                        sendBtnUp);
  }
}
