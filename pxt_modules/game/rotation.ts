namespace sprites {
    ...
    export class RotatedBoundingBox {
        ...
        setRotation(angle: number) {
            ...
            this.points[0] =
                Math.cos(this.cornerAngle + angle) * this.cornerDistance
            this.points[1] =
                Math.sin(this.cornerAngle + angle) * this.cornerDistance
            this.points[2] =
                Math.cos(Math.PI - this.cornerAngle + angle) *
                this.cornerDistance
            this.points[3] =
                Math.sin(Math.PI - this.cornerAngle + angle) *
                this.cornerDistance
            this.points[4] =
                Math.cos(Math.PI + this.cornerAngle + angle) *
                this.cornerDistance
            this.points[5] =
                ...
                this.cornerDistance
            this.points[6] =
                Math.cos(angle - this.cornerAngle) * this.cornerDistance
            this.points[7] =
                Math.sin(angle - this.cornerAngle) * this.cornerDistance
    ...
    function doRectanglesIntersect(
        a: number[],
        ax: number,
        ay: number,
        b: number[],
        ...
        return !(
            checkForNonIntersection(a, ax, ay, b, bx, by) ||
            checkForNonIntersection(b, bx, by, a, ax, ay)
        )
    }

    function checkForNonIntersection(
        a: number[],
        ax: number,
        ay: number,
        b: number[],
        ...
        for (let pointIndex = 0; pointIndex < 4; pointIndex += 2) {
            ...
            for (let i = 0; i < 8; i += 2) {
                const projected =
                    normalX * (a[i] + ax) + normalY * (a[i + 1] + ay)
            ...
            for (let i = 0; i < 8; i += 2) {
                const projected =
                    normalX * (b[i] + bx) + normalY * (b[i + 1] + by)
