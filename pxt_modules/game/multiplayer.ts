namespace multiplayer {
    ...
    export function initServer() {
        if (getOrigin() === "server") {
            game.eventContext().registerFrameHandler(
                scene.MULTIPLAYER_POST_SCREEN_PRIORITY,
                () => {
                    if (getOrigin() === "server") {
                        postImage(screen)
                    }
    ...
    function registerPlayerConnectionListeners(playerNumber: number) {
        control.onEvent(MULTIPLAYER_PLAYER_JOINED_ID, playerNumber, () =>
            receiveConnectionChangedEvent(playerNumber, true),
        )
        control.onEvent(MULTIPLAYER_PLAYER_LEFT_ID, playerNumber, () =>
            receiveConnectionChangedEvent(playerNumber, false),
        )
    }

    function receiveConnectionChangedEvent(
        playerNumber: number,
        connected: boolean,
    ) {
...
namespace mp {
    ...
    export function postPresenceIcon(
        slot: number,
        im: Image,
        implicit?: boolean,
    ) {
        ...
        if (implicit && presenceSetExplicitly[slot]) return
        if (!implicit) presenceSetExplicitly[slot] = true
    ...
    export function postReactionIcon(
        ...
        if (implicit && reactionsSetExplicitly[slot]) return
        if (!implicit) reactionsSetExplicitly[slot] = true
