namespace sprites {
    export class RenderText {
        ...
        drawPartial(
            ...
            for (let i = lineStart; i < lineEnd; i++) {
                currentTextIndex = this.drawPartialLine(
                    canvas,
                    left,
                    top,
                    i,
        ...
        setMaxWidth(maxWidth: number) {
            this.linebreaks = getLineBreaks(this.text, [
                Math.idiv(maxWidth, this.font.charWidth),
            ])
            ...
            for (let i = 0; i < this.linebreaks.length + 1; i++) {
                this.width = Math.max(
                    this.lineEnd(i) - this.lineStart(i),
                    this.width,
                )
            }
        ...
        lineEnd(lineIndex: number) {
            ...
            while (end > prevEnd) {
                if (this.text.charCodeAt(end) <= 32) {
                    ...
                    this.text.charAt(end) === "n" &&
                    this.text.charAt(end - 1) === "\\" &&
                    end - 1 > prevEnd
                ) {
                    end -= 2
                    didMove = true
        ...
        lineStart(lineIndex: number) {
            ...
            while (start < this.text.length) {
                if (this.text.charCodeAt(start) <= 32) {
                    ...
                    this.text.charAt(start + 1) === "n" &&
                    start + 1 < this.text.length
                ) {
                    start += 2
                } else {
                    break
        ...
        widthOfLine(lineIndex: number, fullTextOffset?: number) {
            if (fullTextOffset != undefined) {
                return (
                    (Math.min(this.lineEnd(lineIndex), fullTextOffset + 1) -
                        this.lineStart(lineIndex)) *
                    this.font.charWidth
                )
            }
            return (
                (this.lineEnd(lineIndex) - this.lineStart(lineIndex)) *
                this.font.charWidth
            )
        }
        ...
        widthOfLines(
            ...
            for (
                ...
                i < Math.min(lineEndIndex, this.linebreaks.length + 1);
                i++
            ) {
                if (offset != undefined) {
                ...
                if (
                    fullTextOffset !== undefined &&
                    this.lineStart(i) > fullTextOffset
                )
                    break
    ...
    export class RenderTextAnimation {
        ...
        numPages() {
            ...
            return Math.floor(
                (this.text.linebreaks.length + 1) / maxLinesPerPage,
            )
        }
        ...
        currentHeight() {
            ...
            if (this.state === RenderTextAnimationState.Printing) {
                return Math.max(
                    Math.min(
                        this.text.calculatePartialHeight(
                            this.pageLine,
                            this.currentOffset(),
                        ),
                        maxHeight,
        ...
        currentWidth() {
            return this.text.widthOfLines(
                this.pageLine,
                this.pageLine +
                    Math.idiv(this.currentHeight(), this.text.lineHeight()) +
                    1,
                this.state === RenderTextAnimationState.Printing
                    ? this.currentOffset()
                    : undefined,
        ...
        draw(canvas: Image, left: number, top: number, color: number) {
            if (this.state === RenderTextAnimationState.Idle) return
                ...
                const pageFinished = this.text.drawPartial(
                    ...
                    this.pageLine,
                    this.pageLine +
                        Math.idiv(this.height, this.text.lineHeight()) +
                        1,
                )
