namespace game.consoleOverlay {
    ...
    const consoleLines =
        Math.floor(screen.height / (consoleFont.charHeight + marginy)) - 1
    const consoleColumns = Math.floor(
        (screen.width - 2 * marginx) / consoleFont.charWidth,
    )
    ...
    export function setVisible(value: boolean, col?: number) {
        if (value != !!consoleStrings) consoleStrings = value ? [] : undefined
        if (col !== undefined) consoleColor = col
    }

    function listener(priority: ConsolePriority, text: string) {
        if (!consoleStrings || !text) return
        ...
        for (let i = 0; i < consoleStrings.length; ++i) {
            if (consoleStrings[i].indexOf("\t") >= 0) {
                ...
                for (let tab of t) {
                    ...
                    screen.print(
                        tab,
                        marginx + tOff * consoleFont.charWidth,
                        top + i * height,
                        consoleColor,
                        consoleFont,
                    )
