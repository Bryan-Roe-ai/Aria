namespace particles {
    ...
    export class SprayFactory extends ParticleFactory {
        ...
        createParticle(anchor: ParticleAnchor) {
            ...
            const angle =
                (this.minAngle + galois.randomRange(0, this.spread)) %
                NUM_SLICES
        ...
        setDirection(centerDegrees: number, arcDegrees: number) {
            this.minAngle =
                (toRadians(centerDegrees - (arcDegrees >> 1)) / angleSlice) | 0
    ...
    export class AreaFactory extends SprayFactory {
        ...
        createParticle(anchor: ParticleAnchor) {
            ...
            p.lifespan = this.galois.randomRange(
                this.minLifespan,
                this.maxLifespan,
            )
            p._x = Fx.iadd(
                this.galois.randomRange(0, this.xRange) - (this.xRange >> 1),
                p._x,
            )
            p._y = Fx.iadd(
                this.galois.randomRange(0, this.yRange) -
                    (anchor.height ? anchor.height >> 1 : 0),
                p._y,
            )
        ...
        drawParticle(p: Particle, x: Fx8, y: Fx8) {
            const col = p.lifespan > 500 ? 4 : p.lifespan > 250 ? 5 : 1
    ...
    export class TrailFactory extends ParticleFactory {
        ...
        createParticle(anchor: ParticleAnchor) {
            ...
            p._y = Fx.iadd(
                this.galois.randomRange(0, this.yRange) - (this.yRange >> 1),
                p._y,
            )
    ...
    export class RadialFactory extends ParticleFactory {
        ...
        constructor(
            ...
            else this.colors = [0x2, 0x3, 0x4, 0x5]
        ...
        createParticle(anchor: ParticleAnchor) {
            ...
            const offsetTime =
                (time + this.galois.randomRange(0, this.spread)) %
                cachedCos.length
    ...
    export class AshFactory extends AreaFactory {
        ...
        constructor(
            ...
            updateImage?: boolean,
            percentKept: number = 20,
        ) {
            super(
                anchor.width ? anchor.width : 8,
                anchor.height ? anchor.height >> 1 : 8,
                300,
                700,
            )
        ...
        createParticle(anchor: ParticleAnchor) {
            ...
            p._y = Fx.iadd(
                this.galois.randomRange(this.yRange >> 1, this.yRange),
                p._y,
            )
    ...
    export class BubbleFactory extends ParticleFactory {
        ...
        createParticle(anchor: ParticleAnchor) {
            ...
            p._y = Fx.iadd(
                this.galois.randomRange(-this.yRange, 0) +
                    (anchor.height ? anchor.height >> 1 : 0),
                p._y,
            )
    ...
    export class StarFactory extends ParticleFactory {
        ...
        createParticle(anchor: ParticleAnchor) {
            ...
            p._x = Fx8(
                this.galois.randomRange(anchor.x - xRange, anchor.x + xRange),
            )
            p._y = Fx8(
                anchor.height ? anchor.y - (anchor.height >> 1) : anchor.y,
            )
            ...
            p.lifespan = Fx.toInt(
                Fx.mul(Fx.div(Fx8(screen.height + 20), p.vy), Fx8(1000)),
            )
            ...
            p.data = this.galois.percentChance(15)
                ? this.galois.randomRange(1, this.images.length - 1)
                : 0
    ...
    export class CloudFactory extends ParticleFactory {
        ...
        createParticle(anchor: ParticleAnchor) {
            ...
            if (Math.percentChance(30)) {
                ...
                const selection =
                    Math.randomRange(0, this.clouds.length - 1) << 3

                p.color = isConjoined | isOffsetX | isOffsetY | selection
            }

            p.lifespan = Fx.toInt(
                Fx.mul(Fx.div(Fx8(screen.width + 30), Fx.abs(p.vx)), Fx8(1000)),
            )
        ...
        drawParticle(p: particles.Particle, x: Fx8, y: Fx8) {
            ...
            if (p.color & 1) {
                ...
                const xOffset = isOffsetX
                    ? Fx8(mainImage.width >> 2)
                    : Fx.zeroFx8
                const yOffset = isOffsetY
                    ? Fx8(mainImage.height >> 2)
                    : Fx.zeroFx8
