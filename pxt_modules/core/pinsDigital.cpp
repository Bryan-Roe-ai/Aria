...
bool digitalRead(DigitalInOutPin name) { return PINOP(getDigitalValue()) != 0; }
...
/**
 * Make this pin a digital input, and create events where the timestamp is the
 * duration that this pin was either ``high`` or ``low``.
 */
...
void setPull(DigitalInOutPin name, PinPullMode pull) {
  PullMode m = pull == PinPullMode::PullDown ? PullMode::Down
               : pull == PinPullMode::PullUp ? PullMode::Up
                                             : PullMode::None;
  PINOP(setPull(m));
}
