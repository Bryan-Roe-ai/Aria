...
/**
 * Set the Pulse-width modulation (PWM) period of the analog output. The period
 * is in
...
/**
 * Write a value to the servo to control the rotation of the shaft. On a
 * standard servo, this will set the angle of the shaft (in degrees), moving the
 * shaft to that orientation. On a continuous rotation servo, this will set the
 * speed of the servo (with ``0`` being full-speed in one direction, ``180``
...
/**
 * Set the pin for PWM analog output, make the period be 20 ms, and set the
 * pulse width. The pulse width is based on the value it is given
 * **microseconds** or `1/1000` milliseconds.
 * @param name pin name
