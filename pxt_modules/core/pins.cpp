...
//%
DevicePin *lookupPinCfg(int key) { return lookupPin(getConfig(key)); }

CodalComponent *lookupComponent(int id) {
  for (int i = 0; i < DEVICE_COMPONENT_COUNT; ++i) {
    if (CodalComponent::components[i] &&
        CodalComponent::components[i]->id == id)
 ...
 */
//%
DigitalInOutPin pinByCfg(int key) { return pxt::lookupPinCfg(key); }
...
/**
 * Get the duration of the last pulse in microseconds. This function should be
 * called from a
 * ``onPulsed`` handler.
 */
...
//% weight=19
int pulseDuration() { return pxt::lastEvent.timestamp; }
} // namespace pins
