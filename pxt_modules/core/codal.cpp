 ...
 *
 * @param dest If non-null, it points to an array of pointers to fibers to store
 * results in.
 *
...
// The first two word are used to tell the bootloader that a single reset should
// start the bootloader and the MSD device, not us. The rest is reserved for
// partial flashing checksums.
__attribute__((section(".binmeta"))) __attribute__((used))
const uint32_t pxt_binmeta[] = {
    0x87eeb07c, 0x87eeb07c, 0x00ff00ff, 0x00ff00ff, 0x00ff00ff,
...
void releaseFiber() { release_fiber(); }

void sleep_ms(unsigned ms) { fiber_sleep(ms); }

void sleep_us(uint64_t us) { target_wait_us(us); }
...
void waitForEvent(int id, int event) { fiber_wait_for_event(id, event); }
...
uint64_t getLongSerialNumber() { return device.getSerialNumber(); }

int current_time_ms() { return system_timer_current_time(); }

uint64_t current_time_us() { return system_timer_current_time_us(); }
...
void setThreadContext(ThreadContext *ctx) { currentFiber->user_data = ctx; }

static void *threadAddressFor(codal::Fiber *fib, void *sp) {
  if (fib == currentFiber)
    return sp;
  return (uint8_t *)sp +
         ((uint8_t *)fib->stack_top - (uint8_t *)tcb_get_stack_base(fib->tcb));
}

void gcProcessStacks(int flags) {
  // check scheduler is initialized
  if (!currentFiber) {
    // make sure we allocate something to at least initalize the memory
    // allocator
  ...
  codal::Fiber **fibers =
      (codal::Fiber **)xmalloc(sizeof(codal::Fiber *) * numFibers);
...
void initSystemTimer() { new CODAL_TIMER(*allocateTimer()); }
