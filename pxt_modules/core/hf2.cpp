...
#ifdef USB_EP_FLAG_ASYNC
  HF2_PendingWrite *e =
      (HF2_PendingWrite *)malloc(sizeof(HF2_PendingWrite) + size);
  e->size = size;
  e->flag = flag;
  e->next = NULL;
  uint8_t *dst = e->data;
...
#define checkDataSize(str, add)                                                \
  usb_assert(sz == 8 + (int)sizeof(cmd->str) + (int)(add))
...
const InterfaceInfo *DummyIface::getInterfaceInfo() { return &dummyIfaceInfo; }
...
// waiting for https://github.com/lancaster-university/codal/pull/14
  ...
  // the loop below counts as "system" task, and we don't want to pause
  // ourselves
...
extern "C" void handleHardFault(ExceptionContext *ectx) {
  ...
  DMESG("BB %p %p %p lr=%p r0=%p", instr[-1], instr[0], instr[1], ectx->lr,
        ectx->r0);
